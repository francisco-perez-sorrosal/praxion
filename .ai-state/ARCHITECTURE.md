...
| [dec-draft-e8df5e0b](decisions/drafts/20260424-1550-fperezsorrosal-worktree-tech-debt-integration-tech-debt-ledger-living-artifact.md) (pending finalize) | Tech-debt ledger as living `.ai-state/` artifact | New `.ai-state/TECH_DEBT_LEDGER.md` — single Markdown table with `td-NNN` IDs and 15-field schema (14 row + structural `dedup_key`); append-only rows + status-update-in-place; worktree concurrency handled by post-merge dedupe (`scripts/finalize_tech_debt_ledger.py`, on disk, modeled on `scripts/finalize_adrs.py`); notes-merge separator is ` // ` (reconciled from initial ` | ` spec to avoid Markdown-table column-delimiter collision); cites dec-019/dec-020 (living-doc-in-`.ai-state/`) and dec-013 (no-new-agent). Built end-to-end. |
| [dec-draft-e9a055bb](decisions/drafts/20260424-1550-fperezsorrosal-worktree-tech-debt-integration-tech-debt-producer-integration.md) (pending finalize) | Tech-debt producers — verifier migration + sentinel TD dimension | Verifier Phase 5/5.5 writes ledger rows instead of `LEARNINGS.md ## Technical Debt`; sentinel gains TD dimension (TD01 hotspots, TD02 cyclic SCCs, TD03 coverage floor with stale-OK policy, TD04 complexity p95, TD05 status-update discipline); single owner-role heuristic anchored in `rules/swe/agent-intermediate-documents.md`; cites dec-013 + dec-062–dec-066. Both producers wired and on disk (`agents/verifier.md`, `agents/sentinel.md`); LEARNINGS template `## Technical Debt` section removed in `skills/software-planning/references/document-templates.md`. Built. |
