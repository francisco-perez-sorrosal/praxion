...
| Pipeline Dashboard | Praxion-bundled dashboard service providing a per-project visual entry point. Six surfaces (Architecture / Workshops / ADRs / Sentinel / Roadmap / Metrics) read `.ai-state/` + `.ai-work/<task-slug>/` directly — no new persistence layer; read-only by design. Active runtime is `dashboard_app/` (Next.js App Router + TypeScript) served through `scripts/praxion-dashboard`; deterministic per-project ports remain sha256-derived in the 8501–9500 range; user-scoped Node home lives under `~/.praxion-dashboard/`. Workshops surface keeps mtime-keyed live refresh on a 15 s default interval. Streamlit remains in-tree only as a migration reference until final retirement. macOS daemon v1; Linux/Windows manual-launch documented; systemd v2. Soft-deprecates `metrics-viewer.html.tmpl`. See dec-121..dec-130. | Built | `dashboard_app/`, `scripts/praxion-dashboard`, `commands/dashboard.md`, `streamlit_app/` |
