# Architecture Changelog

Append-only history of `DESIGN.md` verification milestones. Each entry summarizes a feature pipeline that updated the architect-facing design-target document. Owned by systems-architect; appended by the implementer/verifier of each feature.

The current "Last verified" pointer in `.ai-state/DESIGN.md` § 1 is a one-liner referencing the most recent entry below; deep history lives here so the architecture doc stays scannable.

---

## 2026-07-01 — dashboard-debt-trio (Built)

**Pipeline Dashboard** row: renderer registry extended to `resolveRenderer(renderer?, diataxis?, contentType?)` — the manifest `renderer:` field is now the highest-priority lookup key, ahead of the existing `diataxis` → `contentType` → `DefaultShell` fallback chain (all three fallback levels preserved; the new leading param is optional, so every existing call site kept working once updated to pass it through). Three Diátaxis shells (`tutorial`/`how-to`/`concepts`) flipped from `DefaultShell` aliases to dedicated components. Five new per-artifact renderers (`metrics_view`, `plan_view`, `verification_report`, `idea_grid`, `architecture_explorer`) added in a new `src/components/renderers/` package, each reached solely via the `renderer:` key and each degrading to `DefaultShell` on malformed/absent input; `metrics_view`/`architecture_explorer` are deliberately thin Pathway-B previews distinct from the existing Pathway-A full pages (`/metrics`, `/architecture`). No change to `src/server/view-models/documentation.ts` (kept off-limits for the concurrent td-029 hardening pass). Closes td-024/td-025/td-027. Draft ADR `dec-draft-cf359394` (renderer-field-resolver-key) awaiting finalize. `docs/architecture.md` updated in lockstep.

---

## 2026-06-02 — agentic-payment-onboarding (Built)

Agentic-Transactions capability row Designed→Built; `alpaca.md` removed from Key Files (D6-REJECTED); `TRANSACTIONS_DESIGN.md` registered in `rules/swe/agent-intermediate-documents.md`.

---

## 2026-05-13 — doc-arch-sync Option-C dual-agent representation (structural)

LikeC4 model restructured to dual-represent Agents on the two axes Praxion already operates on. The original layer model put `agents` only in the Orchestration layer, conflating authorship (the `.md` files on disk under `agents/`) with runtime (spawned subprocesses with their own context windows). After observing on the rendered components view that Agents were missing from the Knowledge cluster — where they sit semantically alongside Skills / Rules / Commands as a deployable authoring surface — the model was reshaped per Option C: **dual representation**.

**`docs/diagrams/architecture/src/architecture.c4`**:
- New `knowledge.agents` component sibling of `skills`/`rules`/`commands`; description `"Authored subprocess specifications"`.
- Renamed `orchestration.agents` → `orchestration.pipeline`; description `"Spawned subprocesses (runtime instances)"`.
- New edge `knowledge.agents -> orchestration.pipeline "spawns"`.
- Existing knowledge→runtime edges retargeted: `knowledge.{skills,rules,commands} -> orchestration.pipeline` (target rename — they inject/constrain/trigger the runtime, not the authored definitions).
- `orchestration.hooks -> orchestration.pipeline "enforces"`, `orchestration.pipeline -> orchestration.aiwork "reads/writes"`, `orchestration.pipeline -> persistence.aistate "persists"`.
- New edge `tooling.installers -> knowledge.agents "deploys"` — installers deploy the authored agent family alongside skills/rules/commands.
- View `components` `include` list updated for the new node names.

**Regen**: `likec4 gen d2 docs/diagrams/architecture/src -o docs/diagrams/architecture/rendered/` + `d2` per view. `rendered/components.d2` and `rendered/components.svg` updated; `rendered/context.{d2,svg}` and `rendered/index.{d2,svg}` are byte-identical to HEAD (the context view excludes layer internals, and likec4 emits an `index` view that mirrors context here).

**`.ai-state/DESIGN.md` §3**: Knowledge layer now has FOUR components in the model — Skills, Rules, Commands, and **Agents (authored definitions)**. The Orchestration-layer "Agents" row is renamed to "Agent runtime / Pipeline" with the runtime semantics. Added a `>` callout sentence explaining the dual representation and pointing to the draft ADR. §1 "Last verified" line updated.

**`docs/architecture.md` §3**: alt-text on the components.svg image rewritten to mention the four Knowledge components, the renamed Orchestration runtime, and the `spawns` linking edge. The existing "Agents" row is reworded to describe the runtime; a new "Agents (authored definitions)" row is added.

**ADR**: draft `architecture-c4-dual-agent-representation` under `.ai-state/decisions/drafts/` (Context = the original layer model conflated authorship with runtime; user observation flagged Agents missing from the Knowledge cluster in the rendered view; Decision = Option C dual representation; Considered Options = A keep+note / B move-to-knowledge-only / C dual-representation [chosen] / D rename-knowledge; Consequences = model now distinguishes the two axes Praxion already operates on; small uplift in cognitive cost — two related boxes — and a `likec4 gen d2` regen pass). The relaxed node ceiling for LikeC4 (commit `cee12ed`) gave the model room for this structural fix.

## 2026-05-13 — doc-arch-sync (reconciliation, no new decisions)

Reconciliation pass driven by the `doc-arch-sync` documentation+architecture sync pipeline — no behavioral or architectural decisions, only drift correction against ground-truth code. **§3 Components**: Pipeline Dashboard row updated — `streamlit_app/` removed from Key Files and the "retirement-pending reference" prose replaced with "removed in commit `313a50e`; fully migrated to `dashboard_app/`"; the `globals.css` mention now notes the 6-file cascade split (`tokens.css` / `base.css` / `app-chrome.css` / `surfaces.css` / `widgets.css` / `pages-and-viz.css` imported by a minimal `globals.css` `@import` manifest, commit `961371e` / td-030, original intent was a single token file but the monolith breached the 800-line ceiling). Roadmap-cartographer row status `Designed`→`Built` (agent file, `skills/roadmap-synthesis/`, `commands/roadmap.md`, and `.ai-state/ROADMAP.md` all exist). Greenfield-onboarding row file-renames reconciled (`new_cc_project.sh`→`new_project.sh`, `commands/new-cc-project.md`→`commands/new-project.md`, `docs/project-onboarding.md`→`docs/greenfield-onboarding.md`, `tests/new_cc_project_test.sh`→`tests/new_project_test.sh`), with an inline original-intent annotation. **§4 Interfaces**: `new_cc_project.sh` CLI row → `new_project.sh`; `/new-cc-project` slash-command row → `/new-project`; canonical-Praxion-paragraph row provider path → `commands/new-project.md`. **Diagrams**: `docs/diagrams/architecture/src/architecture.c4` agent-count description `"12 specialized agents"`→`"15 specialized agents"`; LikeC4 → D2 → SVG regen run and confirmed a no-op for `rendered/{context,components,index}.{d2,svg}` (`likec4 gen d2` carries `label`s, not `description`s — the agent-count string never reaches a rendered artifact, so no rendered-file change accompanies the `.c4` edit). **`SYSTEM_DEPLOYMENT.md`**: dashboard sections rewritten end-to-end (status `Designed`→`Built`; `streamlit run` / venv replaced with Next.js / Node home `~/.praxion-dashboard/`; Streamlit-specific FMA risk rows replaced with Next.js/Node equivalents; `Last verified` → 2026-05-13). No ADR — pure reconciliation.

## 2026-05-12 — dashboard-redesign (Built)

**Pipeline Dashboard** row in § 3 reflects the completed implementation of the full UX redesign of the Next.js runtime (`dashboard_app/`): a CSS-custom-property design-token layer; a hand-rolled pan/zoom **diagram viewer**; a pure-SVG **ADR relationship graph** (status-colored nodes, supersedes/re-affirms edges, click-to-expand) reused as the workshop step DAG (step N supersedes step N-1, forming a connected chain); a `recharts` **trend-chart** wrapper feeding the metrics charts and a new sentinel health sparkline (historical series parsed from `METRICS_LOG.md` / `SENTINEL_LOG.md`); server-side **SVG sanitization** (`sanitize-html`) for any inline-injected SVG; a `GET /api/diagram?path=<rel>` **route handler** streaming allowlisted SVGs (+ server-side markdown image-ref rewriting so DESIGN.md / architecture.md diagrams render; explicit-diagrams scanner now covers both `docs/diagrams/` and `.ai-state/diagrams/`); a typed **renderer registry** (`Map<diataxis|contentType, ReactComponent>`) with `reference`/`explanation`/`default` Diátaxis shells (documentation page dispatches through it); reusable chrome (`ArtifactCard`, enhanced `EmptyState`, `EducationalPopover`, `Chip`, `Tabs`, `ErrorState`); `validateProjectRoot` relaxed to require only `.ai-state/` (fresh projects work) + an actionable error page for missing `PRAXION_PROJECT_ROOT`; full **Sentrux excision** across `dashboard_app/`, the shipped `metrics-viewer.html.tmpl` (now a static redirect stub, still deployed by `/onboard-project` Phase 4), and Praxion's orphaned `.ai-state/metrics_reports/` files; `tsconfig.json` gains `noUncheckedIndexedAccess`; `parseWipBody` accepts a second checklist format. New deps: `sanitize-html`, `recharts`. `ALLOWED_ARTIFACT_ROOTS` exported from single source (`server/artifacts/project-root.ts`); `MetadataChips` renders summary without truncation.

Seven draft ADRs under `.ai-state/decisions/drafts/`: diagram-serving-and-svg-sanitization, adr-graph-pure-svg (supersedes dec-125), charting-recharts (supersedes dec-130), renderer-registry-and-diataxis-shells, metrics-viewer-stub (supersedes dec-129), validate-project-root-relaxation, design-token-layer. `docs/architecture.md` updated.

## 2026-05-07 — pipeline-dashboard (Designed)

New component **Pipeline Dashboard** added to § 3 — Praxion-bundled Streamlit reader with six surfaces (Architecture, Workshops, ADRs, Sentinel, Roadmap, Metrics) reading `.ai-state/` + `.ai-work/<task-slug>/` directly with no new persistence layer. Bash ctl + macOS launchd lifecycle mirroring `phoenix-ctl`; per-project sha256-derived port mirroring `chronograph-ctl`; dedicated `~/.praxion-dashboard/venv/` isolation. macOS-only v1 with documented manual-launch fallback for Linux/Windows. Soft-deprecates `metrics-viewer.html.tmpl` over one release cycle.

Ten draft ADRs under `.ai-state/decisions/drafts/`: process model, port allocation, cross-platform scope, frontmatter parsing, Mermaid deferral, poll interval, dependency isolation, visualization stack, CLAUDE.md placement, metrics-viewer supersession. Implementation deferred to implementation-planner spawn; `docs/architecture.md` not yet updated — Built status pending.

## 2026-05-03 — ai-training-onramp (Designed)

Third project archetype — ML/AI training — added to the catalog with three new components (Project archetype catalog, ML training subsystem, Neo-cloud abstraction), three new interfaces (`training_job_descriptor`, `TRAINING_RESULTS.md`, `program.md`), and three new constraints (descriptor mode-invariance, ML training-loop determinism waiver, mandatory compute-budget declaration). Five draft ADRs under `.ai-state/decisions/drafts/` covering verifier extension, tiered backend strategy, `program.md` placement, skill scope, and `TRAINING_RESULTS.md` schema ownership. Implementation deferred to implementation-planner spawn.

## 2026-04-26 — praxion-first-class (Built)

Shipped: `hooks/inject_subagent_context.py` + `hooks/inject_process_framing.py` + `hooks/auto_complete_install.py` + `scripts/render_claude_md.py` + § Praxion Process canonical block in `commands/onboard-project.md`/`commands/new-project.md`. Three draft ADRs under `.ai-state/decisions/drafts/` awaiting finalize.

## 2026-04-24 — project-metrics + tech-debt-integration (Built)

`/project-metrics` slash command + `scripts/project_metrics/` package + `docs/metrics/README.md` schema reference + `docs/metrics/index.html` trend-visualization page; five `/project-metrics` draft ADRs awaiting finalize.

Tech-debt-integration shipped end-to-end: `.ai-state/TECH_DEBT_LEDGER.md` empty artifact + 15-field schema in `rules/swe/agent-intermediate-documents.md` + `scripts/finalize_tech_debt_ledger.py` post-merge dedupe + hook-chain wiring in `scripts/git-post-merge-hook.sh` + verifier Phase 5/5.5 ledger writes + sentinel TD01–TD05 dimension + 5 consumer-contract additions across systems-architect / implementation-planner / implementer / test-engineer / doc-engineer + `## Technical Debt` removed from LEARNINGS template. Three draft ADRs under `.ai-state/decisions/drafts/` finalize to stable `dec-NNN` at merge-to-main.
