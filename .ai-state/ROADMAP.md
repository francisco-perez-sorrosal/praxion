...
## P6 — [Budget discipline] Conservative "relocate-don't-delete" pass on the big always-loaded rules · ☑ done (2026-05-12)

> **Done 2026-05-12** (commit `b5c6a60`; ADR-free, no architect needed). A `context-engineer` audit (`.ai-work/relocate-rules/CONTEXT_REVIEW.md`) walked the four big always-loaded files section-by-section against the three-step protocol and found **zero relocations needed** — every verbose passage already lives in a reference file the rule cites (`coordination-details.md`, `adr-authoring-protocols.md`, `tech-debt-ledger.md`, `agent-pipeline-details.md`) or in the linked GitHub issue / `td-NNN` ledger row. So the pass is **five compress-in-place changes, zero new files, zero signal lost**:
> 1. `CLAUDE.md` § Known Claude Code Limitations — the 3 verbose bug entries → the working summary each (symptom + workaround + `td-NNN` + primary issue links; diagnostic narrative stays in the issues); section anchor preserved.
> 2. `CLAUDE.md` § When NOT to use the full pipeline — the tier-table duplicate → a one-line pointer to `swe-agent-coordination-protocol.md` § Process Calibration; the "default to the lower tier" principle stays.
> 3. `rules/swe/swe-agent-coordination-protocol.md` § Coordination Pipeline — tightened the Doc-engineer + Interface-designer table rows; dropped the redundant per-row `coordination-details.md` link (the section intro already links it).
> 4. `rules/swe/agent-intermediate-documents.md` § `.ai-state/` — converted the 11-paragraph per-artifact prose to a compact table (`artifact | writers/updaters | shape·lifecycle·reference`); all template paths, section-ownership notes, and the dev-vs-architect-doc distinction preserved; the `#tech_debt_ledger-summary` anchor + the `#### TECH_DEBT_LEDGER` subsection untouched; the `decisions/` writers list brought in line with `adr-conventions.md` (added `interface-designer`).
> 5. `rules/swe/adr-conventions.md` § Re-affirmation Protocol — the closing rationale paragraph (verbatim in `adr-authoring-protocols.md` § Re-affirmation Protocol) → the load-bearing "use it only when…" clause + a pointer.
>
> **Always-loaded surface (project `CLAUDE.md` + the `paths:`-less rules, `README.md` excluded): 70,637 → 68,150 bytes — −2,487 bytes ≈ −690 tokens (/3.6).** Net of the P2+P3+P5 additions (~+200 tokens): **~−490 tokens net** — meets the roadmap's "net delta ≤ 0" target (net-negative, restoring headroom; less than the audit's optimistic −1,585 estimate because the rules were leaner than estimated once "zero signal lost" was strictly enforced — all issue links, all template paths, one file-type example each, and the re-affirmation guardrail clause were kept rather than maximizing bytes). `sync_canonical_blocks.py --check` green (no canonical block touched); `pytest tests/` + `test_finalize_adrs.py` green; `/sentinel` (report `SENTINEL_REPORT_2026-05-12_14-30-00.md`) — Health B / Coherence A, **0 new findings from P6** (anchors intact, the new `adr-authoring-protocols.md#re-affirmation-protocol` link resolves, token-efficiency improved), 2 pre-existing Important findings (the `git-post-merge-hook.sh`→`git-finalize-hook.sh` doc reference; missing installed `.git/hooks/finalize`). Executed in worktree `relocate-rules`; this Done note is the "what moved where" record the roadmap's "done when" calls for.
