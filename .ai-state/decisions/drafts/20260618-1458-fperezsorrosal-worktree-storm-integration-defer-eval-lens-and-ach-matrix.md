...
superseded_by: dec-draft-60a83db4
...
1. **Full ACH matrix** — superseded by the DI + two-tier Disconfirmation scheme (dec-draft-60a83db4), which covers the deliberation need at lower cost and with better evidence.
...
This deferral's ACH clause is superseded by dec-draft-60a83db4 (DI + Disconfirmation), which is the chosen replacement mechanism. The eval-lens clause stands as a live deferral.
...
- **Reversal trigger**: Build the agentic eval-lens when (a) a `project_profile.yaml` with `archetype: agentic_ai` exists in a managed project, OR (b) the first managed agentic project enters the pipeline; reintroduce ACH per dec-draft-60a83db4's own reversal trigger.
