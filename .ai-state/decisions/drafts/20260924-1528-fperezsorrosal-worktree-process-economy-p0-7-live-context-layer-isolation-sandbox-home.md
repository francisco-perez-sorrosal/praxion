...
- **Detection beyond prevention:** a Bash allowlist glob (`cat *`, `tail *`) cannot tell stdin from `~/.ssh/…`. Every tool_use input is therefore scanned for a path that resolves (as given or through `realpath`) under the operator's real home directory — the asset isolation protects — and any hit classifies the session as `isolation_breach` from telemetry. A first version flagged every absolute path outside the sandbox and the copy; it tripped on `/dev/null`, markup and the session's own `/private/var` files (three of four real recorded sessions), so the check names what it protects rather than everything it does not expect.
