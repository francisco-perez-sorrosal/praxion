...
- [ ] **AC-07**: TypeScript coding-style guidance is delivered with path/activation scoping (frontmatter `paths: ["**/*.ts", "**/*.tsx", "**/*.mts", "**/*.cts"]`), and its absence from the always-loaded surface is verified by the implementation planner before merge. *(As shipped 2026-05-11 this was a dedicated path-scoped rule; folded into `skills/typescript-development/contexts/typescript.md` on 2026-05-13, commit `57013be`.)*
...
|----|-------------|---------------------|--------------|--------|
| AC-01 | Polyglot Skill Template ADR on disk | Pre-condition (architect Phase 2); `dec-139` | ADR exists at `.ai-state/decisions/139-polyglot-skill-template.md` (draft finalized to this path at merge-to-main); `python3 scripts/finalize_adrs.py --all --dry-run` exits 0; step-19 PASS | PASS |
| AC-02 | Frontend Framework Nesting ADR on disk | Pre-condition (architect Phase 2); `dec-137` | ADR exists at `.ai-state/decisions/137-frontend-framework-nesting.md` (draft finalized to this path at merge-to-main); finalize dry-run exits 0; step-19 PASS | PASS |
| AC-03 | Angular Exclusion ADR on disk | Pre-condition (architect Phase 2); `dec-135` | ADR exists at `.ai-state/decisions/135-angular-exclusion-from-contexts.md` (draft finalized to this path at merge-to-main); finalize dry-run exits 0; step-19 PASS | PASS |
...
| AC-05 | Zod v3/v4 split has canonical home in `skills/node-prj-mgmt` | step-01, step-13 | `grep "overrides" skills/node-prj-mgmt/contexts/typescript.md` returns match (step-01 done-when); step-13 adds cross-ref to `skills/agentic-sdks/contexts/openai-agents-typescript.md`; `dec-140` ADR documents the decision; step-19 PASS | PASS |
| AC-06 | Complete artifact creation order in the implementation plan | Implementation plan itself (steps 01–21 with dependency graph and parallel groups A–F) | The ephemeral `.ai-work/multi-language-support/IMPLEMENTATION_PLAN.md` existed with 21 steps and a parallel-group table; deliverable was the planning document itself (consumed and cleaned at archival) | PASS |
| AC-07 | TS coding-style guidance has path/activation scoping; not always-loaded | step-12 | `grep -q "^paths:"` on the as-shipped path-scoped rule exited 0; step-18 measured always-loaded surface — excluded; verdict **AC-07 PASS**. Guidance now lives in `skills/typescript-development/contexts/typescript.md` (folded 2026-05-13, `57013be`) | PASS |
