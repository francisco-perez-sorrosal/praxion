...
| REQ | Description | Tests | Implementation | Status |
|-----|-------------|-------|-----------------|--------|
| REQ-01 | Trigger only on `ecosystem-feedback` + non-Bot sender | `test_arming_gate_requires_exact_label_and_non_bot_sender`, `test_trigger_is_issues_labeled_only` | `issue-autofix.yml`::job `if:` (label==ecosystem-feedback && sender.type!=Bot) | PASS |
| REQ-02 | Any other label or Bot sender is a structural no-op | `test_arming_gate_conjuncts_are_joined_by_and_not_or`, `test_agent_cannot_self_arm_via_its_own_label_applications` | `issue-autofix.yml`::job `if:` (structural skip) | PASS |
| REQ-03 | Per-issue concurrency, no cancel-in-progress | `test_concurrency_group_is_scoped_per_issue`, `test_concurrency_never_cancels_an_in_flight_run` | `issue-autofix.yml`::top-level concurrency group | PASS |
| REQ-04 | Daily run budget enforced | `test_daily_budget_gate_step_exists` | `issue-autofix.yml`::step "Enforce daily run budget" | PASS |
| REQ-05 | Idempotency guard on terminal labels / open PR | `test_idempotency_guard_checks_terminal_labels_or_open_pr` | `issue-autofix.yml`::step "Skip if already triaged..." | PASS |
| REQ-06 | Untrusted body/title fetched+sanitized, read as DATA | `test_issue_body_fetch_step_is_non_agent`, `test_raw_issue_body_and_title_never_appear_in_a_prompt_block`, `test_fetch_output_is_sanitized_before_the_agent_reads_it`, `test_prompt_frames_issue_content_as_untrusted_data` | `issue-autofix.yml`::step "Fetch and sanitize issue body" | PASS |
| REQ-07 | Missing required §5.2 sections -> `triage:invalid`, stop | `test_template_validate_step_references_the_triage_module`, `test_malformed_issue_gets_triage_invalid_label`, + 6 `issue_triage.py` unit tests (`parse_sections`/`missing_required_sections`) | `issue_triage.py`::`parse_sections()`, `missing_required_sections()`, `REQUIRED_SECTIONS`; `issue-autofix.yml`::step "Template-validate and dedup" | PASS |
| REQ-08 | Fingerprint/dedup match -> `duplicate`, stop | `test_duplicate_issue_gets_duplicate_label`, `test_dedup_search_excludes_the_current_issue`, + 7 `issue_triage.py` unit tests (`extract_fingerprint`/`dedup_signature`) | `issue_triage.py`::`extract_fingerprint()`, `dedup_signature()`; `issue-autofix.yml`::step "Template-validate and dedup" | PASS |
| REQ-09 | Reproduction contained to an allowlisted, non-blanket Bash set | `test_allowed_tools_declares_no_blanket_shell_pattern`, `test_allowed_tools_is_an_enumerated_git_gh_pytest_set`, `test_allowed_tools_excludes_gh_issue_create`, `test_allowed_tools_excludes_gh_run_view`, `test_allowed_tools_never_grants_a_blanket_git_push`, `test_no_step_pushes_directly_to_the_default_branch`, `test_prompt_forbids_pushing_to_the_default_branch` | `issue-autofix.yml`::fixer step `claude_args.allowedTools` + prompt REPRODUCTION section | PASS |
| REQ-10 | Mechanical fix -> `issue-autofix/<n>-<slug>` PR, `Fixes #<n>` | `test_prompt_references_the_mechanical_fix_pr_path`, `test_prompt_distinguishes_mechanical_from_behavioral` | `issue-autofix.yml`::fixer prompt MECHANICAL path | PASS |
| REQ-11 | Behavioral/architectural -> root-cause comment + `needs-adr`, no PR | `test_prompt_references_the_behavioral_needs_adr_path` | `issue-autofix.yml`::fixer prompt BEHAVIORAL/ARCHITECTURAL path | PASS |
| REQ-12 | Deny-by-default on governance surfaces | `test_prompt_names_the_deny_by_default_governance_surfaces` | `issue-autofix.yml`::fixer prompt CLASSIFY section | PASS |
| REQ-13 | Sensitive-path fix -> draft + human-review warning | `test_sensitive_path_tripwire_step_exists`, `test_sensitive_path_tripwire_targets_the_issue_autofix_branch_prefix`, `test_sensitive_path_tripwire_toggles_draft_via_gh_pr_ready_undo` | `issue-autofix.yml`::sensitive-path tripwire step | PASS |
| REQ-14 | No `track_progress`; status via `gh` comment/edit only | `test_never_references_track_progress` | `issue-autofix.yml`::fixer step (no track_progress) | PASS |
| REQ-15 | All action refs SHA-pinned, matching P1 | `test_every_uses_reference_is_sha_pinned`, `test_checkout_pin_matches_p1_exact_commit`, `test_setup_uv_pin_matches_p1_exact_commit`, `test_claude_code_action_pin_matches_p1_exact_commit` | `issue-autofix.yml`::checkout/setup-uv/claude-code-action pins | PASS |
| REQ-16 | Fixer job holds exactly the four declared permissions | `test_fixer_job_permissions_are_exactly_the_four_declared_grants`, `test_no_job_grants_actions_permission`, `test_no_job_grants_broader_permissions_than_declared` | `issue-autofix.yml`::job permissions block | PASS |
| REQ-17 | Every `issue-autofix/*` PR reviewed by P2 cross-model gate | — (no P5 code by design) | — | PASS-by-composition (P2's `reusable-cross-model-review.yml` `agent-prs` scope already matches `issue-autofix/*`; realized once `feat/self-healing-p2` merges to `main`) |
| REQ-18 | No AI-authorship lines in fixer commits | `test_prompt_forbids_ai_authorship_commit_lines` | `issue-autofix.yml`::fixer prompt COMMIT HYGIENE section | PASS |
