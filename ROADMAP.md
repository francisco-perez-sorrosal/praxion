...
- **FW-1** ✅ DONE (2026-04-16) — Sentinel drift check shipped as `EC06` in `agents/sentinel.md` under Ecosystem Coherence. L-type (LLM judgment), unconditional, runs in Phase 4 Batch 3 alongside BC02. Scoped to **outputs only** (the condensed block lists deliverables produced; the rule's "Read X" input clauses are out of scope), with explicit shorthand tolerance (e.g., "architecture doc validation" matches "ARCHITECTURE.md + docs/architecture.md"). On today's state the check PASSes — baseline captured. D1-A Option 4 is now covered by automated audit rather than a future ADR.
- **FW-2** ⚠️ PARTIALLY DONE (2026-04-16) — Closed 3 of the 6 deferred Lightweight gaps via gap-analysis between Lightweight and Standard tier coverage: (a) mid-task escalation procedure (grow-beyond-3-files → re-scope, not silent creep), (b) ADR creation at Lightweight (`made_by: user`), (c) calibration-log entry parity. The original `RESEARCH_FINDINGS.md` Section 6 that enumerated the six gaps by number was cleaned with `.ai-work/coord-burden-reduction/` post-pipeline, so exact mapping to gap numbers 3/6/7/9/10/11 is not recoverable — 3 high-signal gaps identified through Lightweight-vs-Standard contrast. Remaining 3 gaps (whatever they were) stay deferred as low-priority; if they matter, they will resurface as concrete pain.
- **FW-3** ✅ DONE (2026-04-16) — `re-affirmation` promoted to a first-class ADR status value ahead of the trigger condition (user override). Schema changes: added `re-affirmation` to the `status` enum in `rules/swe/adr-conventions.md`; added `re_affirms: dec-NNN` and `re_affirmed_by: [dec-NNN]` relationship fields parallel to `supersedes`/`superseded_by`; added a Re-affirmation Protocol section documenting the 6-step procedure. Applied to the canonical first use case: dec-049 `status` flipped from `accepted` to `re-affirmation`; dec-049 gained `re_affirms: dec-022`; dec-022 gained `re_affirmed_by: [dec-049]`; `DECISIONS_INDEX.md` regenerated. Known caveat: still a single occurrence — schema complexity may prove over-built if the pattern never recurs.
- **FW-4** ✅ DONE (2026-04-16) — Added two new scaffolds to `skills/software-planning/references/tier-templates.md`: **Full-Tier Multi-Instance Fan-Out** (N instances of same role over disjoint file sets, e.g., three implementers on three modules) and **Full-Tier Speculative Execution** (N variants of same role exploring alternative approaches, coordinator synthesizes). Extended the Placeholder Reference table with four new tokens (`<instance-k>`, `<instance-total>`, `<variant-label>`, `<alternative>`). DRY Boundary section unchanged — scaffolds link to the authoritative Delegation Checklists. Same caveat as FW-3: neither pattern has a live use case yet; the scaffolds are speculative templates that may need adjustment when a real case emerges.
