...
- **FW-2** — Close the six deferred Lightweight gaps (3, 6, 7, 9, 10, 11). **Status**: still open, priority lowered. The research's own classification flagged these as low-severity (vs. the five high-severity gaps closed in D3). Evidence caveat: the source `RESEARCH_FINDINGS.md` Section 6 enumerating each gap was cleaned with `.ai-work/coord-burden-reduction/` post-pipeline — pursuing FW-2 now requires re-deriving the gap list via fresh researcher spawn. Defer until accumulated pain or a broader Lightweight v2 pass justifies the re-investigation cost.
- **FW-3** — Promote `re-affirmation` to a formal ADR status value. **Status**: deferred pending trigger. Current re-affirmation count = 1 (ADR-049 only); trigger fires at 2nd or 3rd occurrence. Pre-emptive promotion would invent process ahead of evidence and contradict the simplicity-first rationale dec-049 itself applied. Revisit when a second ADR re-affirms a prior decision rather than superseding it.
- **FW-4** — Tier-templates.md additions for emerging delegation patterns. **Status**: deferred pending trigger. Multi-instance fan-out and speculative execution are named as candidates but neither has an active use case or design. Templating the hypothetical risks the over-extraction dec-022/dec-049 pushed back against. Revisit when one of these patterns appears in an actual pipeline run and needs a reusable prompt shell.
