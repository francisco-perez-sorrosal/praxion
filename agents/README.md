...
| `doc-engineer` | Maintains project-facing documentation quality (README.md, catalogs, architecture docs, changelogs); runs in parallel with implementer and test-engineer on planner-assigned doc steps; validates cross-references, catalog completeness, naming consistency, writing quality, and visual-tone discipline (emoji/badge/color); loads `web-ui-design` on-demand for HTML share-outs and diagram-heavy docs; routes deep API-doc production to `api-documentation` | `doc-management`, `web-ui-design`, `api-documentation` |
| `sentinel` | Independent read-only ecosystem quality auditor scanning every context artifact and the persistent `.ai-state/` corpus against the check catalog in its own definition — per-artifact quality, code health, decision and spec health, gate liveness, architecture completeness, and ecosystem coherence as a system-level composite (the catalog is the authoritative dimension list; no count is restated here, because a numeral drifts from the table on the first addition); produces timestamped `SENTINEL_REPORT_*.md` (accumulates) and `SENTINEL_LOG.md` (historical metrics) in `.ai-state/sentinel_reports/` — any agent or user can consume its reports | — |
...
| `agentic-transactions-architect` | Transaction-domain expert and shadow + on-demand sub-architect for managed projects implementing agentic payments or agentic trading; shadows researcher and systems-architect stages when a task involves transaction / payment / trading / brokerage / mandate / settlement / HITL context, producing `TRANSACTIONS_DESIGN.md` with design decisions, provider-contract analysis, HITL spend-gating, and architecture challenges; registers objections via the orchestrator-mediated challenge loop; runs standalone transaction design reviews | `agentic-transactions`, `external-api-docs`, `mcp-crafting`, `agentic-sdks` |
| `discipline-consultant` | Discipline-parameterized adversarial consultant — a gated peer sub-architect giving existing knowledge standing to object. Resolves a `Discipline: <name>` directive against the data-only roster in `skills/multi-perspective-analysis/references/discipline-registry.md`; writes falsifiable, decision-naming challenges to `CONSULT_<discipline>.md` for per-challenge disposition; adversarial-only (challenges, never decides); no production code, no ADRs | `multi-perspective-analysis` (discipline knowledge binds at runtime via the `Skill` tool, so this column stays the same no matter how many disciplines the registry carries) |
...
| `agentic-transactions-architect` | Transaction domain expert (shadow + on-demand sub-architect) | Shadows researcher + architect stages when agentic payments/trading is in scope; orchestrator-mediated **challenge loop** with architect via `## Architecture Challenges` in `TRANSACTIONS_DESIGN.md` |
| `discipline-consultant` | Discipline-parameterized adversarial consultant | Gated spawn — nominated by pipeline agents or convened by orchestrator/user; writes challenges to `CONSULT_<discipline>.md`; orchestrator-mediated **challenge loop** with architect via challenges and disposition |
