...
8. **Verify the architecture is sufficient (handoff completeness gate)** — before decomposing, confirm `SYSTEMS_PLAN.md` carries enough design detail to plan against. Check:
   - Are the acceptance criteria expressed as observable behaviors, not implementation directives?
...
Commits stay pathspec-scoped throughout: when the orchestrator commits a batch's work, each commit names the specific files a single agent/step produced (`git commit <files...>`), never a broad `git add -A`/`git add .` spanning the whole batch. This keeps each concurrent agent's contribution independently attributable and revertible, and is what makes disjoint `Files:` lists enforceable in practice rather than just declared on paper.
