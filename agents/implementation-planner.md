...
1. **Prepare the batch** — write WIP.md in parallel mode with per-step assignees and file lists. The main agent spawns concurrent agents with `isolation: "worktree"` for intra-pipeline isolation (see [Pipeline Isolation](../rules/swe/swe-agent-coordination-protocol.md#pipeline-isolation)). When worktree isolation is used, each agent works in its own copy and branches are merged after the batch. As a fallback when worktree isolation is unavailable, instruct each agent to write to fragment files (`WIP_<agent-type>.md`, `LEARNINGS_<agent-type>.md`, `PROGRESS_<agent-type>.md`) per the [agent-intermediate-documents](../rules/swe/agent-intermediate-documents.md) naming convention. A batch may include up to three agents: implementer, test-engineer, and doc-engineer.
