...
4. **Carry signals into Phase 4 — each with its warrant attached.** Use external signals to (a) ground new ideas in adjacent-project traction, (b) avoid ideas the landscape has already invalidated, (c) surface evolution-trend opportunities the project's internal state alone would not reveal.

   **Every signal you carry forward must record what it is and what it does *not* license.** A landscape signal is an *imported claim*: someone else's assertion, arriving with the authority of having been published. Write it as `<claim> — <source and its evidentiary class> — <what this does not establish>`. A peer project shipping a pattern establishes that *they* shipped it, not that it worked, not that it transfers here. A blog post's benchmark is a self-run benchmark unless stated otherwise. A standards draft is a proposal, not a commitment.

   This is the cheapest point in the pipeline to attach a warrant and the most expensive point to omit one: an unwarranted signal that seeds an idea is relayed by research, absorbed into architecture, and only questioned — if ever — after the design rests on it. Load the `evidence-appraisal` skill when a signal is load-bearing for an idea you are about to propose; the source-strength, transfer, and citation-drift material there is what turns a fetched sentence into a claim a later reader can re-appraise. You are **not** running an appraisal consult here — ideation is the divergence phase and a critic inside it suppresses the variety this stage exists to produce. You are recording provenance so the convergence phases downstream can do their job.
