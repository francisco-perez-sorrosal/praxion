...
| T03 | A | Agent prompt size within range | Under 400 lines (warn 300, fail 500). **Exception**: the two gate-encoding agents `agents/sentinel.md` and `agents/verifier.md` are assessed at warn 550 / fail 700 — their size is substantially intrinsic (the sentinel's `## Check Catalog` is ~260 rows of distinct checks executed inline; the verifier's comprehensive Process plus its tested-in-place rework-spawn contract). Forcing either under 500 risks a dead gate or guts tested substance. All other agents use the standard thresholds. |
...
| P07 | A | Undisposed Architecture Challenges in specialist design docs | For each `.ai-work/<slug>/INTERFACE_DESIGN.md` and `.ai-work/<slug>/TRANSACTIONS_DESIGN.md`, grep for a non-empty `## Architecture Challenges` section. Flag **Important** when the section exists and is non-empty but no disposition follows it (no `Status:`, `Decision:`, or `Resolved:` line before the next `##` heading) — a specialist raised a load-bearing question the pipeline never answered. Also cover each `.ai-work/<slug>/CONSULT_<discipline>.md`'s `## Challenges` section (the discipline-consultant's deliberately distinct heading — never `## Architecture Challenges`): flag **Important** when a `### CH-NN` entry's `**Disposition:**` field is empty or still the `<!-- convener, Round 2 -->` placeholder before the next `##` heading — the convener never adjudicated a challenge the consultant raised. Skip with a P-dimension INFO note when none of `INTERFACE_DESIGN.md`/`TRANSACTIONS_DESIGN.md`/`CONSULT_*.md` is present, or when every challenge section/entry carries a disposition. Golden bad-case: `tests/fixtures/sentinel/challenge_no_disposition/INTERFACE_DESIGN.md` (non-empty challenges, no disposition → flag) and `tests/fixtures/sentinel/consult_no_disposition/CONSULT_statistician.md` (non-empty `## Challenges`, undispositioned `### CH-01` → flag); no-false-positive controls: `INTERFACE_DESIGN_no_challenge.md` and `CONSULT_statistician_no_challenge.md` (no challenges section → no flag). |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| TD01 | L | Hotspots warrant ledger entries | Read `METRICS_REPORT_*.md` `hotspots` (churn × complexity); apply LLM-judgment gating; write one row per warranted item per § Producer overlays → **sentinel** → TD01 |
| TD02 | L | Non-trivial cyclic SCCs are structural defects | Read `METRICS_REPORT_*.md` `pydeps.cyclic_sccs`; for each SCC of size > 1, write one row per § Producer overlays → **sentinel** → TD02 |
| TD03 | L | Coverage below project floor | Read `METRICS_REPORT_*.md` `coverage` namespace; for each module under the project floor (default 70% when no project threshold is set), write one row per § Producer overlays → **sentinel** → TD03. `coverage.status = stale` triggers the staleness WARN above; produce findings from available data, never block |
...
| DL04 | L | No orphaned supersession, re-affirmation, or retirement pointers | Finalized ADR with `supersedes: dec-NNN` / `superseded_by: dec-MMM` / `re_affirms: dec-NNN` / `re_affirmed_by: [dec-MMM]` / `retired_by: [dec-MMM]`: referenced file must exist under `.ai-state/decisions/`. Draft ADR with `supersedes: dec-draft-<hash>` / `re_affirms: dec-draft-<hash>`: referenced draft file must exist under `.ai-state/decisions/drafts/`. Mixed pointers — a finalized ADR pointing at `dec-draft-<hash>`, or a draft pointing at a `dec-NNN` it could not have legitimately known at authoring time — are a WARN (finalize should have rewritten them) |
...
| AC11 | A | The design doc's structural components and the architecture model project the same set | Run `python3 scripts/check_architecture_projection.py --json`. Each finding is a **FAIL**: `element-without-row` (a modelled component the design doc never documents), `row-without-element` (a documented component carrying no structural enforcement — the shape that let most of the table drift), `unknown-element` (the doc names an element the model does not declare, typically a rename the doc did not follow), `not-structural` (a row for a layer container, which double-counts its children). Resolving one is a judgment about *which side is right*: the model wins when its edges encode enforced structure, the doc wins when the model is over-granular. Skip with an AC-dimension INFO note when either the model or the design doc is absent. Golden bad-case: a component element added to the model with no corresponding row |
