...
| S06 | L | Agent prompts use numbered stages with completion criteria | Each agent's work is decomposed into **numbered stages, each with a stated completion criterion** — the substance being tested. The label is not: this repo carries at least three equally rigorous carriers, and a literal grep for any one of them false-flags agents that satisfy the intent completely. `## Phase N` is the majority form (15 agents); `agents/discipline-consultant.md` uses `### Round N`, which is the correct vocabulary for an adversarial consult that iterates rather than advances; `agents/implementer.md` uses an explicitly ordered list (`1.` … `7.6.`) under a single `## Execution Workflow` heading, which is correct for a linear per-step workflow that would gain nothing from seven headings. Accept any of these; judge whether the stages are **numbered and terminated**, not how they are named. A check that reports correct work as drift teaches its reader to skip the dimension, which costs more than the check returns (the same reasoning that narrowed EC03 and retired AC11). Golden bad-case: an agent whose process is unnumbered prose with no stated point at which any stage is done — flag that, and only that |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| P03 | A | Agent lifecycle records pair: every `agent_start` has a matching `agent_stop` | Read `.ai-state/observations.jsonl` (JSONL, one record per line) and pair `event_type: agent_start` with `event_type: agent_stop` **on `agent_id`** — the only field that identifies a single spawn. Pairing on `agent_type` is the trap: concurrent instances share a type, so one sibling's stop silently satisfies another's start and the fleet reports clean. WARN per unpaired start. **Two boundaries are not findings**, and a check that reports either teaches its reader to skip the dimension: (i) an agent still in flight legitimately has a start and no stop — exclude every record whose `session_id` equals that of the newest record in the file; (ii) the WAL is truncated at the front by rotation and by `.ai-state/` merges, so an `agent_stop` whose start predates the file's first line is a boundary artifact — count unmatched *stops* as INFO, never WARN. Report the number of records examined beside the finding count: zero findings over three records is not a clean fleet, and only the denominator tells them apart (the discipline T02 and TD02 apply to their own numbers). Skip with a P-dimension INFO note when the file is absent. Golden bad-case: an `agent_start` from a session other than the newest whose `agent_id` appears in no `agent_stop` — the check must WARN naming that `agent_id` and its `agent_type`, where a type-pairing check passes it because a sibling instance of the same type did stop |
...
### Pipeline Discipline
[P03/P04: findings plus the count of WAL records examined — a zero over three records is not a clean fleet. P05-P08 from `.ai-work/`. When skipped, name which of the three substrate states applies; never a bare "unavailable"]
