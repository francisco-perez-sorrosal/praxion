...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| C01 | A | Every skill dir has `SKILL.md` | Family: `python3 scripts/check_artifact_conformance.py --json`; Every `skills/*/` dir has a `SKILL.md`. |
| C02 | A | Every `SKILL.md` has `description` in frontmatter | Family: `python3 scripts/check_artifact_conformance.py --json`; Every `SKILL.md` declares `description`. |
| C03 | A | Every agent `.md` has `name`, `description`, `tools` in frontmatter | Family: `python3 scripts/check_artifact_conformance.py --json`; Every `agents/*.md` declares `name`/`description`/`tools`. |
| C04 | A | Every command has a non-empty `description` field or header comment | Family: `python3 scripts/check_artifact_conformance.py --json`; Every command has a non-placeholder `description` or header comment. |
| C05 | A | `plugin.json` lists all agents by file path | Family: `python3 scripts/check_artifact_conformance.py --json`; plugin.json agents count equals `agents/*.md` file count. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| N01 | A | Skill dirs follow naming conventions | Family: `python3 scripts/check_artifact_conformance.py --json`; Skill dirs are kebab-case with the right `-crafting`/`-development` suffix. |
| N02 | A | Agent files follow naming conventions | Family: `python3 scripts/check_artifact_conformance.py --json`; Agent file stems are kebab-case. |
| N03 | A | Frontmatter uses valid keys per spec | Family: `python3 scripts/check_artifact_conformance.py --json`; SKILL.md/agent frontmatter uses only spec-recognized keys. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| F01 | A | Referenced files exist on disk | Family: `python3 scripts/check_path_resolution.py --json`; WARN per `findings` entry with `check: "F01"` — a declared-limit detector; see docstring for accepted false positives. |
| F02 | A | Skill `references/` files exist | Family: `python3 scripts/check_path_resolution.py --json`; WARN when a skill's own `references/*.md` mention does not exist under that skill (declared limit: cross-skill prose mentions fire too). |
...
| F05 | A | `SYSTEM_DEPLOYMENT.md` referenced file paths exist | Family: `python3 scripts/check_path_resolution.py --json`; FAIL per `findings` entry with `check: "F05"`. |
...
| F07 | A | Cataloged section missing marker | Family: `python3 scripts/check_staleness_markers.py --json`; WARN per `check: "F07"` finding — a cataloged heading has no marker below it. |
| F08 | A | Marker age > threshold | Family: `python3 scripts/check_staleness_markers.py --json`; WARN per `check: "F08"` finding past threshold; FAIL beyond 2× threshold. |
| F09 | A | Marker invalid format / future-dated | Family: `python3 scripts/check_staleness_markers.py --json`; FAIL per `findings` entry with `check: "F09"` — unparseable or future-dated, excluding the `_`-template `[YYYY-MM-DD]` placeholder. |
| F10 | A | Git hook source matches installed copy(ies), resolved by content | Family: `python3 scripts/check_hook_installation.py --json`; WARN per `findings` entry — a git hook source, resolved by content not filename, matching no installed hook or one that differs. |
| F11 | A | `doc_manifest.yaml` is fresh vs the surfaces it indexes | Family: `python3 scripts/check_doc_manifest_freshness.py --json`; WARN when `generated_at` predates the newest add/delete/rename under `docs/`/`.ai-state/` (excluding this manifest's own regen commit); body-only edits and the regen commit itself never WARN; refresh with `python3 scripts/build_doc_manifest.py`. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| S01 | A | `SKILL.md` frontmatter has `description` | Family: `python3 scripts/check_artifact_conformance.py --json`; Every SKILL.md `description` is present and non-empty. |
| S02 | A | Agent frontmatter has `name`, `description`, `tools` | Family: `python3 scripts/check_artifact_conformance.py --json`; Every agent `name`/`description`/`tools` are present and non-empty. |
| S03 | A | Rule files start with `##` heading | Family: `python3 scripts/check_artifact_conformance.py --json`; Every rule file's first non-blank body line is a `##` heading. |
| S04 | A | Commands have `$ARGUMENTS` when expected | Family: `python3 scripts/check_artifact_conformance.py --json`; Every command declaring `argument-hint` substitutes it in content. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| X01 | A | plugin.json agent paths resolve | Family: `python3 scripts/check_registry_projection.py --json`; FAIL when a plugin.json agents-array path does not resolve to an existing `.md` file. |
| X02 | A | plugin.json skill/command dirs contain loadable artifacts | Family: `python3 scripts/check_registry_projection.py --json`; FAIL when a skill dir has no `SKILL.md` or a command file is empty. |
| X03 | A | CLAUDE.md `## Structure` dirs exist | Family: `python3 scripts/check_path_resolution.py --json`; FAIL per `findings` entry with `check: "X03"`. |
| X04 | L | Idea ledger implemented ideas reference real artifacts | Implemented ideas correspond to artifacts that exist |
| X05 | A | Agent roster matches `agents/` | Family: `python3 scripts/check_registry_projection.py --json`; FAIL when the Agent Roster table and `agents/*.md` disagree in either direction. |
| X06 | A | `agents/README.md` table matches `agents/` | Family: `python3 scripts/check_registry_projection.py --json`; FAIL when agents/README.md's agent table and `agents/*.md` disagree in either direction. |
...
| X09 | A | Deployment doc ADR cross-references valid | Family: `python3 scripts/check_path_resolution.py --json`; FAIL per `findings` entry with `check: "X09"` — dec id has no finalized ADR file. |
...
| T02 | A | Combined always-loaded content size, plus the listing (skill/command/agent `description:` frontmatter) surface | Family: `python3 scripts/measure_token_budget.py --json`; FAIL only when `examined.basis == "tokenizer"` and `over_by > 0` (or listing over ceiling); `ratio` basis (no API key) reads as **INFO** ("unmeasured"), never FAIL. `listing_tokens` is advisory — `check_token_ratchet.py` gates it. No divisor here — see `rules/CLAUDE.md § Token Budget`. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| P03 | A | Agent lifecycle records pair: every `agent_start` that ran has a matching `agent_stop`, reported per `session_id` | Family: `python3 scripts/check_agent_lifecycle_pairing.py --json`; WARN per `findings` entry naming `agent_id`/`agent_type`/`session_id`; report `info.incidents`/`pre_baseline`/`no_tool_use` as separate counts; report `unmatched_stops` (`unobserved-start`/`unobserved-agent`/`unattested`) and `producer_log_disagreement` separately. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| TT01 | A | Every group's `subsystems` values resolve to a `Status: Built` component in `.ai-state/DESIGN.md` §3a | Family: `python3 scripts/check_topology_conformance.py --json`; FAIL per `check: "TT01"` finding — a group's `subsystems` entry absent from, or not Status: Built in, the §3a table. |
| TT02 | A | Every `selectors` entry has a registered `strategy` identifier | Family: `python3 scripts/check_topology_conformance.py --json`; FAIL/WARN per `check: "TT02"` finding — an unregistered strategy (FAIL) or one documented optional in the leaf (WARN). |
...
| TT05 | A | Marker-name consistency and reserved-name set compliance | Family: `python3 scripts/check_topology_conformance.py --json`; FAIL per `check: "TT05"` finding — a marker-selector group id unregistered in the pocket's marker config, or colliding with the reserved-name set. |
| TT06 | A | A topology-less project that has grown past the adoption thresholds should be told to adopt one | Family: `python3 scripts/check_topology_conformance.py --json`; Runs whether or not `.ai-state/TEST_TOPOLOGY.md` exists; `check: "TT06"` skips when present, otherwise emits an INFO advisory naming each Growth-Trigger threshold confirmed crossed. Never writes the topology file or a ledger row. |
| TT07 | A | Every corpus `test_*.py` file is claimed by exactly one TEST_TOPOLOGY.md group selector | Family: `python3 scripts/check_topology_conformance.py --json`; FAIL per `check: "TT07"` finding — an orphaned (uncovered), overlapping (claimed twice), or dangling-selector `test_*.py` under `tests/`/`scripts/`/`hooks/`/`fitness/`. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| EC01 | A | Pipeline diagram agents have files | Family: `python3 scripts/check_registry_projection.py --json`; FAIL when a pipeline-diagram node name has no matching `agents/*.md` file. |
| EC02 | A | No orphaned artifacts | Family: `python3 scripts/check_registry_projection.py --json`; WARN when a skill, command, or rule is referenced by no agent, CLAUDE.md, rule, command, skill or doc (own files excluded). |
...
| EC07 | A | AaC golden-rule drift in recent commits | Family: `python3 scripts/check_aac_golden_rule.py --mode=audit --json`; Important per `check: "EC07"` finding — a generated artifact drifted from its source over the last N commits (default horizon 10); one row per § Producer overlays → **sentinel** → EC07; WARN overall when any exist. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| SH01 | A | Persistent specs reference files that exist | Family: `python3 scripts/check_state_corpus.py --json`; WARN/FAIL per `check: "SH01"` finding — a non-conforming or dangling path-column reference. |
| SH02 | A | Persistent specs have traceability matrices | Family: `python3 scripts/check_state_corpus.py --json`; FAIL per `check: "SH02"` finding — no Traceability matrix carrying a data row. |
...
| SH07 | A | Spec↔artifact drift detected against current HEAD | Family: `python3 scripts/check_spec_drift.py --json`; emit its rows as-is — the wrapper already maps severity and defers `orphaned-edge` to SH01/SH04; never Critical; unique value: `stale-dependent` + `untracked-req` kinds. See `sentinel-spec-checks.md § SH07`. |
| SH08 | A | Spec-archival gap — recent ADRs without a paired archived spec | Family: `python3 scripts/check_spec_archival_gap.py --json`; Important per `check: "SH08"` finding — newest archived spec >90 days older than a cluster of ≥3 finalized ADRs sharing a tag. |

### Calibration Accuracy (CA)

Requires `.ai-state/calibration_log.md`; every CA check is a family row, and the family skips with a CA-dimension INFO note when the log is absent (CA02 also below 5 classified rows).

| ID | Tp | Rule | Pass |
|----|----|------|------|
| CA01 | A | `calibration_log.md` exists with valid table format | Family: `python3 scripts/check_state_corpus.py --json`; FAIL per `check: "CA01"` finding — header mismatch or zero data rows. |
| CA02 | A | Retrospective enum distribution over the last 20 post-cutover rows stays within band | Family: `python3 scripts/check_calibration_coverage.py --json`; WARN when over- or under-calibrated exceed 25% of classified rows or every classified row reads `correct` (zero-variance alarm), and per non-canonical token; INFO skip below 5 classified rows. |
| CA03 | A | Calibration coverage — recent task commits are actually logged | Family: `python3 scripts/check_calibration_coverage.py --json`; WARN when `covered` is false (≥2 task-completing commits since the newest row — the append has no enforcing hook); INFO skip when the log is absent. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| RD01 | A | Agent readiness below production floor | Family: `python3 scripts/check_readiness_feedback.py --json`; Important per `check: "RD01"` finding — `examined.adjusted_level` below the Practiced production floor (3); annotate when `examined.mechanical_only`. Writes no `td-NNN` row. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| DL01 | A | `.ai-state/decisions/` has ADR files in either lifecycle stage when archived specs exist | Family: `python3 scripts/check_state_corpus.py --json`; WARN per `check: "DL01"` finding — archived specs exist with no ADR in either lifecycle stage. |
| DL02 | A | ADR files have valid YAML frontmatter with required fields, **all present and non-empty** | Family: `python3 scripts/check_state_corpus.py --json`; FAIL per `check: "DL02"` finding — an ADR missing or emptying a required frontmatter field. |
| DL03 | A | `DECISIONS_INDEX.md` is consistent with finalized ADRs only | Family: `python3 scripts/regenerate_adr_index.py --check --json`; Each `findings[]` entry is a **WARN** -- the index is stale against the ADR corpus; disposition is to run without `--check` to regenerate. |
...
| DL06 | A | Cross-reference pointers are reciprocal (both directions set) | Family: `python3 scripts/check_adr_reciprocity.py --json`; WARN per missing-back-link finding, naming both ids; **FAIL** per unreadable-frontmatter finding (a record DL06 cannot parse cannot be cross-referenced); `retired_by` is exempt. |
...
| AC03 | A | File paths in `.ai-state/DESIGN.md` are illustrative | Family: `python3 scripts/check_architecture_projection.py --json`; WARN when >50% of §3a Key Files paths in `.ai-state/DESIGN.md` do not resolve. |
| AC04 | A | Inline `dec-NNN` references in `.ai-state/DESIGN.md` and `docs/architecture.md` resolve | Family: `python3 scripts/check_architecture_projection.py --json`; WARN per `findings` entry with `check: "AC04"` -- unresolved `dec-NNN` reference. |
| AC05 | A | `docs/architecture.md` exists and is non-empty when `.ai-state/DESIGN.md` exists | Family: `python3 scripts/check_architecture_projection.py --json`; WARN per `findings` entry with `check: "AC05"` -- missing/empty developer guide. |
| AC06 | A | Every component name in developer guide matches actual module | Family: `python3 scripts/check_architecture_projection.py --json`; WARN per `findings` entry -- a §3a row's Key Files anchor has no top-level match. |
| AC07 | A | File paths in developer guide resolve | Family: `python3 scripts/check_architecture_projection.py --json`; WARN per `findings` entry -- a §3a Key Files path does not resolve on disk. |
...
| AC13 | A | The design doc projects both of its authorities — the architecture model and the shipped-block registry | Family: `python3 scripts/check_architecture_projection.py --json`; Each finding is a **FAIL**; read `withheld` — unreadable registry withholds published half, not zero. |
| AC14 | A | `.ai-state/DESIGN.md`'s `Current as of` checkpoint is not stale against the ADR corpus | Family: `python3 scripts/check_design_checkpoint.py --json`; advisory per `check: "AC14"` finding reporting `examined.count`, never FAIL — the mark advances only on the systems-architect's explicit per-ADR judgment; FAIL only when `examined.checkpoint_state` is `malformed`/`absent`. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| HK01 | A | Hackathon mode graduation nudge when project exceeds PoC size | Family: `python3 scripts/check_hackathon_graduation.py --json`; Skips with `skipped.HK01 = {"reason": "hackathon-mode-off"}` when `PRAXION_HACKATHON_MODE` is unset/0. Otherwise INFO advisory naming source-file and commit counts when either crosses threshold (>40 files OR >150 commits). |
...
| GL02 | A | No forbidden-pattern contradiction: an instruction greps/asserts a pattern another rule forbids | Family: `python3 scripts/check_gate_liveness.py --json`; FAIL per `check: "GL02"` finding — an instruction searches test names for a `req{NN}_` prefix or another pattern a rule forbids; the check is dead. <!-- gate-liveness:ignore --> |
| GL04 | A | No uninvoked gate: a gate nothing calls catches nothing | Family: `python3 scripts/check_gate_liveness.py --json`; FAIL per `check: "GL04"` finding — a gate no hook, command, agent, workflow or sibling script invokes. |
| GL05 | A | No unloadable gate: an interpreter that cannot import the gate runs nothing | Family: `python3 scripts/check_gate_liveness.py --json`; FAIL per `check: "GL05"` finding — a gate an ambient `python3` cannot import; the finding names the packages it needs. |
| GL06 | A | No discarded verdict: a gate whose result the surface it runs on cannot transmit | Family: `python3 scripts/check_gate_liveness.py --json`; FAIL per `check: "GL06"` finding — a gate whose findings exit code its registration surface structurally cannot transmit. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| DH01 | A | Decisions whose subject was removed by a *later* decision carry a supersession link | Family: `python3 scripts/adr_health.py --json`; WARN per `check: "DH01"` finding — a decision whose subject a later decision deleted, with no supersession link between them; disposition is to confirm the remover and record the link. |
| DH02 | A | Reference decay is repaired, not left to accumulate | Family: `python3 scripts/adr_health.py --json`; WARN per `check: "DH02"` finding — a `renamed`, `placeholder-shape` or `out-of-repo` reference (`fix-entry`/`update-path`); read `withheld` first. |
| DH04 | A | A retired decision whose subject returns is re-opened, not left terminal | Family: `python3 scripts/adr_health.py --json`; WARN per `check: "DH04"` finding — a `retired` decision whose named path resolves again; verify the subject, then flip `status` and clear `retired_by`. |
| DH05 | A | The `architectural` category still discriminates | Family: `python3 scripts/adr_health.py --json`; Suggested per `check: "DH05"` finding, emitted only on a `widening` verdict; `examined` carries the verdict, `post_adoption_n` and the architectural share. |
| DH06 | A | Partial-supersession state is internally consistent | Family: `python3 scripts/adr_health.py --json`; WARN per `check: "DH06"` finding — a partial-supersession edge clash; the finding names its `shape` (a–e) and `disposition`. |
...
| ID | Tp | Rule | Pass |
|----|----|------|------|
| V01 | A | Sentinel registered in plugin.json | Family: `python3 scripts/check_sentinel_self_audit.py --json`; FAIL when `.claude-plugin/plugin.json`'s `agents` array omits `./agents/sentinel.md`. |
| V02 | A | Sentinel in the Agent Roster table | Family: `python3 scripts/check_sentinel_self_audit.py --json`; FAIL when the Agent Roster table in `coordination-details.md` has no `sentinel` row. |
| V03 | A | Sentinel in `agents/README.md` | Family: `python3 scripts/check_sentinel_self_audit.py --json`; FAIL when `agents/README.md`'s agent table has no `sentinel` row. |
| V04 | A | Check catalog present **and populated** | Family: `python3 scripts/check_sentinel_self_audit.py --json`; FAIL when `## Check Catalog` is absent, has no check-ID rows, or any `###` dimension heading has none beneath it — the golden bad-case is a dimension heading left standing after its rows were cut mid-edit. |
...
**Family dispatch (auto).** Run `python3 scripts/run_check_families.py --table` **once**: it executes every family in the table below and prints one digest — per check id its FAIL/WARN/INFO counts, `skipped`, `examined`, `bound` and sample entities. Use the digest rows as each check's evidence; a check whose `skipped` is set gets an INFO note in its dimension; reproduce `bound` verbatim as the PASS statement. Drill into one family's own `--json` only when a finding needs more entities than the digest shows. Spec + golden bad-cases for every family script: its module docstring and canary `scripts/test_<name>.py`.
...
| `.ai-state/decisions/` | `python3 scripts/adr_health.py --json` | DH01 DH02 DH04 DH05 DH06 |
...
| `.ai-state/TEST_TOPOLOGY.md` — run regardless; TT06 reads its absence | `python3 scripts/check_topology_conformance.py --json` | TT01 TT02 TT05 TT06 TT07 |
...
| `.ai-state/metrics_reports/` | `python3 scripts/check_readiness_feedback.py --json` | RD01 |
| `.ai-state/specs/` | `python3 scripts/check_spec_archival_gap.py --json` | SH08 |
| `.ai-work/` | `python3 scripts/check_p06_task_brief.py --json` | P06 |
| `scripts/check_aac_golden_rule.py` present | `python3 scripts/check_aac_golden_rule.py --mode=audit --json` | EC07 |
| `.ai-work/` | `python3 scripts/clean_work_safety.py --json` | P08 |
| always (governed 9-file set) | `python3 scripts/measure_token_budget.py --json` | T02 |

When ≥1 architecture markdown file exists (AC10 substrate trigger), include AC10 in this pass: `python3 scripts/aac_fence_validator.py <file>` per in-scope architecture markdown; skip with AC-dimension INFO note when substrate absent. GL01 and GL03 are LLM-judgment checks — run them in Pass 2. DH03 is LLM judgment — run it in Pass 2.
