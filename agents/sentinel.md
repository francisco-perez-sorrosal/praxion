...
**Tooling:** AC10 invokes `scripts/aac_fence_validator.py`; AC12 invokes MCP `query-by-metadata`; neither introduced a new validator. AC13 invokes `scripts/check_architecture_projection.py`, which shipped with it. **AC12's MCP path is not reachable from this agent's tool grant** (`Read, Glob, Grep, Bash, Write` — no MCP). It is dormant rather than broken only because its substrate is unpopulated; whoever populates the bidirectional convention must give AC12 a Bash-reachable reader first, or it will report against an input it cannot fetch.
