...
**Four conditional-activation checks (AC10, AC11, AC12, AC13)** extend the AC dimension following the TT idiom: each check has a substrate-presence trigger; absence skips the check with an AC-dimension INFO note ("ACNN skipped — `<substrate>` not present"). Never WARN or FAIL on substrate absence alone.
...
**Tooling:** AC10 invokes `scripts/aac_fence_validator.py`; AC11 invokes MCP `read-project-summary` (with grep fallback); AC12 invokes MCP `query-by-metadata` — that trio introduced no new validators. AC13 invokes `scripts/check_architecture_projection.py`, which shipped with it.

AC11 and AC13 both compare the model against the design doc and are deliberately distinct: AC11 is LLM judgment over *component names* via a symmetric diff, tolerant of naming drift; AC13 is a deterministic reconciliation over the *element ids* each row declares, and also covers the published half (§4 canonical-block rows ↔ the shipped-block registry) that AC11 does not see.

**Paired site.** `docs/aac-dac.md` carries a reader-facing subset of this table — the AaC-relevant checks, AC10–AC13. Adding or retiring one of those means updating that document in the same commit; this table is the source of truth, but it is not the only site a reader learns the set from.
