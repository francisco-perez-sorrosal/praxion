...
| AC04 | A | Inline `dec-NNN` references in `.ai-state/DESIGN.md` and `docs/architecture.md` resolve | Run `python3 scripts/check_architecture_projection.py --json`. WARN per `findings` entry with `check: "AC04"` -- unresolved `dec-NNN` reference. Spec + golden bad-cases: `scripts/check_architecture_projection.py` docstring; canary `scripts/test_check_architecture_projection.py`. |
| AC05 | A | `docs/architecture.md` exists and is non-empty when `.ai-state/DESIGN.md` exists | Run `python3 scripts/check_architecture_projection.py --json`. WARN per `findings` entry with `check: "AC05"` -- missing/empty developer guide. Spec + golden bad-cases: `scripts/check_architecture_projection.py` docstring; canary `scripts/test_check_architecture_projection.py`. |
