...
Conditional activation: skip GL02 and GL04 with a GL-dimension INFO note when `scripts/check_gate_liveness.py` is absent (substrate trigger, TT idiom — never WARN/FAIL on substrate absence). Both come from the same `--json` run. GL01 and GL03 are LLM judgment and run whenever `agents/` exists.
