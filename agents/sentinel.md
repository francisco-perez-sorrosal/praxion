...
**Family dispatch (auto).** For every family script in the table: run it once, route each `findings[]` entry to the catalogue row named by its `check` key at that entry's `severity`, read `skipped` first (keyed by `<id>`; DL03's is flat) and report a check that could not run as an INFO note in that row's dimension, and reproduce `bound` (keyed the same way) verbatim as the row's PASS statement. Skip a family with an INFO note in its dimension when its substrate is absent. Spec + golden bad-cases for every family script: its module docstring and canary `scripts/test_<name>.py`.
