...
5. **`decisions.jsonl`** -- check `.ai-state/decisions.jsonl` for decisions that have already been captured as rules, skills, or memory entries to avoid proposing artifacts that duplicate existing knowledge.
