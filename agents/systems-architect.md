...
4. **Clarify the goal** — if `.ai-work/<task-slug>/TASK_BRIEF.md` exists, its `## Task Intent` is the user's authoritative goal (captured at the Intake Clarity Gate) — restate from it, don't re-derive. Otherwise restate the goal in one sentence; if ambiguous, state your interpretation and ask for confirmation.
5. **Define acceptance criteria as behavioral specs** — concrete, testable conditions for "done" expressed as observable behaviors. **When `TASK_BRIEF.md` exists, seed these from its `## Key Signals`** (each Key Signal is already a binary outcome predicate) and its `## Uncertainty Flag` (a flag below 7/10 warrants a spike or extra validation step). These criteria drive test design downstream: the test-engineer will derive behavioral tests directly from them. Write each criterion as a verifiable behavior ("When X happens, the system does Y"), not an implementation detail ("Module Z is refactored").
...
7. **Detect brownfield baseline** — for Standard and Full tier tasks, check `.ai-state/specs/` for prior `SPEC_*.md` files relevant to the feature being designed. If found, note the prior spec as the behavioral baseline for delta production in Phase 6. Also consult `.ai-state/decisions/DECISIONS_INDEX.md` for ADRs overlapping the current task's scope. **Prefer `scripts/query_adrs.py`** for a scope keyword or tag lookup — it queries the corpus directly and needs no manual `offset`/`limit` bookkeeping. Fall back to grep-pre-scanning `DECISIONS_INDEX.md` only when the query script is unavailable: the index grows unbounded with the decision corpus and can run tens of thousands of tokens, so run `grep -in '<keyword>' .ai-state/decisions/DECISIONS_INDEX.md` for each scope keyword (tags, affected paths, feature terms), read only the matching rows (via `offset`+`limit`), then read the full ADR files for relevant matches. Read the entire index only when the task is broad enough that a keyword scan would miss cross-cutting decisions. Prior decisions constrain the design space and should be acknowledged, not silently contradicted. If no prior specs or decisions exist, the feature is greenfield.
...
**Full authoring procedure** — the magnitude×independence classifier that chooses among the four outcomes, the tech-debt-ledger consumer-only filter/flip protocol, the `PRE_REFACTOR_PLAN.md` 8-section schema, and the HOW-vs-WHEN/WHAT boundary with [`skills/refactoring/SKILL.md`](../skills/refactoring/SKILL.md) — lives in [`skills/software-planning/references/coordination-details.md` § Architect-Side Authoring Procedure](../skills/software-planning/references/coordination-details.md#architect-side-authoring-procedure).
...
- Creativity at design level, convention at line level — for load-bearing decisions, generate at least two genuinely different shapes before committing, including a reframing that might delete the need for new code entirely; a novel structure is admissible when its premise is documented and its concrete payoff named (fewer states, fewer edge cases, smaller reasoning surface — elegance as a functional claim, never aesthetic authority). Line-level cleverness remains banned; the clever-vs-insightful test lives in the `beautiful-code` skill (cite `CLAUDE.md§Structural Beauty`)
- Make the architecture testable — if it can't be tested, redesign it
- If the design introduces an agent or MCP surface, load `agentic-sdks`/`communicating-agents`/`mcp-crafting` as appropriate — these are on-demand loads, not statically injected
...
**Checkpoint duty.** Run `python3 scripts/check_design_checkpoint.py --json` before finishing this section. Read its `unfolded` list against the architectural decisions you just finalized in Phase 7: for each un-folded ADR, decide whether it is already reflected in §1–§3 ("folded in") or does not bear on the design-target picture ("not applicable") — **both dispositions are resolved the same way: advance §1's `Current as of` row to the highest `dec-NNN` you dispositioned this way.** Advancing the mark is *itself* the disposition for either outcome, never a separate step gated on rewriting prose; the validator has no way to distinguish "reviewed and folded in" from "reviewed and judged not applicable," and neither needs to distinguish them — both mean the mark can safely move past that id. **Never advance the mark mechanically or by default** — an un-folded ADR you have not actually read and judged stays un-folded, and the mark stays where it is; a mark advanced without that judgment is a mechanized lie about what the design doc has actually absorbed.
