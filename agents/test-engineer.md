...
description: >
  ...
  with expert-level strategy. Use when testing work exceeds the implementer's
...
6. **Write test results** — after running the full test suite, write `.ai-work/<task-slug>/TEST_RESULTS.md` using the fixed shape in `skills/software-planning/references/agent-pipeline-details.md § TEST_RESULTS.md Reconciliation`: command, a `Result: pass=<n> fail=<n> skip=<n>` line, duration, and the optional topology lines. A green section (`fail=0`, no `error=`>0) carries nothing further — no notes field, no coverage summary (coverage lives in `coverage.xml`, not the section). A red section adds the log pointer and one `### Failures` block per failing test. Presence of the file is the handoff signal to the verifier. **Canonical-writer rule**: when paired with an implementer on the same step (BDD/TDD execution), the test-engineer is the canonical writer of `TEST_RESULTS.md` — the implementer skips its own Post-implementation updates step (write test results). In parallel mode, write fragment `TEST_RESULTS_test-engineer.md` — the planner merges fragments by concatenating `## Step N` sections in ascending step order. **When the step carries `mutation: on`** (see [`decomposition-guide.md § Step Risk Tagging`](../skills/software-planning/references/decomposition-guide.md#step-risk-tagging)), the canonical writer additionally runs `scripts/mutation_sensor.py --targets <files> --tests <files>` and copies its stdout `Mutation:` line **verbatim** into this section — never re-rendered. Absent the tag or `mutation: off`, nothing runs and nothing is written.
