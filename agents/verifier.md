...
When a specialist artifact exists in `.ai-work/<task-slug>/` — `INTERFACE_DESIGN.md` (task involved an interface surface: web UI, TUI/CLI output, API, MCP tools), `TRANSACTIONS_DESIGN.md` (task involved agentic payments or trading), `ARCHITECTURE_VALIDATION.md` (the architect-validator ran), or any `CONSULT_<discipline>.md`:
