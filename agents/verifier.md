...
When a specialist artifact exists in `.ai-work/<task-slug>/` — `INTERFACE_DESIGN.md` (task involved an interface surface: web UI, TUI/CLI output, API, MCP tools), `TRANSACTIONS_DESIGN.md` (task involved agentic payments or trading), `ARCHITECTURE_VALIDATION.md` (the architect-validator ran), or any `CONSULT_<discipline>.md` — or when `SYSTEMS_PLAN.md § Architecture` carries a populated `### Data Structures` section, or the diff introduces or alters domain types, state shapes, or component/agent schemas (representation surface):

- For a representation surface, read `skills/data-structure-design/references/design-review-checklist.md` on demand (that skill is not injected into this agent) and audit the diff against it; when `### Data Structures` is populated, also verify the implemented shapes match the declared invariants and enforcement points — a declared invariant with no enforcing code is a FAIL-grade mismatch. The checklist's own sections carry the golden bad-cases these findings must flag.
...
- For each in-scope hat, run its `design-review-checklist.md` (e.g., `web-ui-design/references/design-review-checklist.md`, `api-design-craft/references/design-review-checklist.md`). The four interface-design skills are loaded on demand when the interface hat is in scope — read the relevant checklist directly. For `TRANSACTIONS_DESIGN.md`, read `agentic-transactions/references/design-review-checklist.md` on demand (that skill is not injected into this agent). Its `## Quick Verdict Guide` names the FAIL-grade items; treat an irreversible operation with no human gate and no reasoned exemption, or a spend cap whose only enforcement is prompt text, as FAIL rather than WARN. When `supports_sandbox` or the approval mode is absent from the design doc, several items are unevaluable — report those as WARN (`unevaluable`), never as PASS.
...
- For `ARCHITECTURE_VALIDATION.md` you are the **named reader of its non-FAIL half**. Its FAIL findings already left as `TECH_DEBT_LEDGER` rows and need no re-filing; its WARNs, its overall verdict, and any section it could not evaluate exist nowhere else and die with `.ai-work/`. Carry each still-unresolved WARN into `VERIFICATION_REPORT.md` as a WARN, and read the verdict for what it does *not* say: a WARN meaning a check could not run (no model present, a tool unavailable, a config missing) is a coverage gap, not an all-clear, so never let a `PASS_WITH_WARNINGS` verdict stand in your report as structural confirmation. Structure remains the validator's judgment — reproduce and route its findings, do not re-derive or overrule them. **A missing `ARCHITECTURE_VALIDATION.md` is not a finding**: the validator is not a standard pipeline stage and most runs have none.
...
`REWORK_MANIFEST.md` is a hybrid markdown table + per-row fenced JSON format (JSON authoritative). Table columns: `#`, `Worktree`, `Agent`, `Severity`, `Tier`, `Class`, `Headline`. Each row's `id` is an `rw-<8-hex>` hash from `scripts/rework_manifest.py:compute_row_id(report_id, cluster_signature)` — stable across re-runs given the same inputs.

Full clustering algorithm, row-ID derivation, dedup check, the row-dict JSON schema, the `VERIFIER_FINDINGS.md` seven-section template, the disposition-vocabulary pointer, and the write-failure error grammar: [`skills/software-planning/references/rework-manifest.md`](../skills/software-planning/references/rework-manifest.md).
...
After `REWORK_MANIFEST.md` is written, the main agent (not the verifier) spawns one `EnterWorktree` per row, writes each rework worktree's `VERIFIER_FINDINGS.md`, snapshots the parent `VERIFICATION_REPORT.md` into it, flips linked `td-NNN` ledger rows `open → in-flight`, and surfaces `/resume-rework` to the user. The verifier's responsibility ends with writing `REWORK_MANIFEST.md` — it does not flip ledger rows or spawn worktrees itself.

Full main-agent-facing protocol (per-row worktree naming, the write contracts, the dispatch one-liner, parent-cleanup gating, and the resolution path at merge): [`skills/software-planning/references/rework-manifest.md`](../skills/software-planning/references/rework-manifest.md).
...
`VERIFICATION_REPORT.md` is an optional input to skill-genesis's pattern harvesting, invoked downstream and user-initiated — see [`agents/skill-genesis.md` § With the Verifier](skill-genesis.md#with-the-verifier).
