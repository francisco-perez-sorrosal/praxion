...
- **Onboarding blocks** — no longer embedded in any command: the unification moved onboarding to `skills/onboard-project/`, and the canonical `## Agent Pipeline`, `## Compaction Guidance`, `## Behavioral Contract`, and `## Praxion Process` blocks now have exactly one embedding site, `skills/onboard-project/references/claude-md-blocks.md`. User-facing doc: `docs/onboarding.md`.
