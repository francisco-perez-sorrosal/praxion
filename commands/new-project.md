...
| 8 | Stage scaffold + print exit handoff (recommends `/onboard-project` — or `/onboard-project --hackathon` in hackathon mode — then `/co`) | None — terminal phase |
...
9. **What to read next** — four short bullets in this exact order: (1) "Run `/onboard-project` (or `/onboard-project --hackathon` when this project was created in hackathon mode — its Phase 5b then writes the five remaining hackathon artifacts without prompting) to apply the remaining onboarding surfaces — git hooks (ADR finalize, id-citation discipline), merge driver for `.ai-state/observations.jsonl`, `.ai-state/` skeleton, `.claude/settings.json` toggles. The greenfield scaffold leaves these for `/onboard-project` so both onboarding paths share one source of truth.", (2) "Run `/co` to make your first commit (or `/cop` for commit+push); both apply `rules/swe/vcs/git-conventions.md` automatically, so you don't hand-craft commit messages.", (3) "(Optional) Create `.claude/praxion-rules.yaml` to disable any rules your team doesn't need (hook-deliver or path-scoped; core rules are protected) — see `docs/rules-taxonomy.md` for the taxonomy and token accounting. No config file = all rules load (backward compatible).", (4) "Open `docs/greenfield-onboarding.md` in the Praxion repo for the full greenfield walkthrough; `docs/existing-project-onboarding.md` covers the existing-project path that `/onboard-project` drives."
...
This block is installed into the user project's `CLAUDE.md` by Flow step 10g (when Obsidian integration is enabled) and by Phase 8d sub-step 8d.5 in `/onboard-project`. It is guarded by `grep -q '^## Obsidian Integration$' CLAUDE.md`. The fence is kept byte-identical to `claude/canonical-blocks/obsidian-integration.md` by `scripts/sync_canonical_blocks.py`.
...
The mushi doc's "What to read next" carries the same step-1 language — `/onboard-project` first (or `/onboard-project --hackathon` when hackathon mode is enabled; smooth integration with the existing-project path), `/co` second (commit), and a pointer to `docs/greenfield-onboarding.md` (this command's companion doc) plus `docs/existing-project-onboarding.md` (the `/onboard-project` companion).
