...
**Predicate.** The four core blocks (Agent Pipeline, Compaction Guidance, Behavioral Contract, Praxion Process) delegate their skip/refresh decision to `refresh_claude_blocks.py`'s own absent/current/stale/modified classification (see **Action** below) — there is no separate heading-check predicate for them. Two independent heading checks remain for the blocks outside `REFRESHABLE_SLUGS`:
