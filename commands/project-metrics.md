...
- **Cost block.** Every run writes a `cost` namespace at the root of the JSON report and a `## Cost` Markdown section. The `CostCollector` reports token usage per pipeline, per calibration tier, and per agent type — honest-provenance rows only (rows carrying `usage_source: "subagent-transcript"`); every other population (`parent-sourced`, `pre-attribution`, `unparsed`, `summary-rollup-unattributed`) is counted by name in `coverage.quarantine`, never folded into a total. A `coverage: N attributed / M total` line opens the section. The report deliberately withholds a Standard-vs-Lightweight verdict: the `f12` cell renders `n/a` with a reason unless both tiers carry attributed rows. See `docs/metrics/README.md` § 5.8 for the full JSON shape.
