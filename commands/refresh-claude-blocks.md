---
description: Refresh this project's onboarded CLAUDE.md canonical blocks against the currently installed i-am plugin, with an interactive disposition loop for locally customized blocks
...
**currently installed** i-am plugin. Each block is classified `current` /
...
Run this after upgrading the i-am plugin, or any time `/onboard-project`
...
Get the live i-am install path from `installed_plugins.json` — the script must
...
If that is null/empty, tell the user the i-am plugin is not installed and
