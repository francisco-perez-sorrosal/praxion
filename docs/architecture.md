...
| Artifact registry | The single source of truth for the `.ai-work/<slug>/` artifact set: a typed `ARTIFACTS` tuple whose projection helpers the three hard-coded consumer lists are checked against by `scripts/test_artifact_registry.py` (drift gate + gate-liveness canaries). Each `Artifact` declares a `production_gate` (`"<kind>:<ref>"`, kind ∈ `_GATE_KINDS`) naming the gate that makes it exist and a `cleanup_policy` (∈ `_CLEANUP_POLICIES`) governing its removal. These two fields are populated, not projected — the consumer drift assertions do not read them. | `scripts/artifact_registry.py`, `scripts/test_artifact_registry.py` |
