...
| **Commands** | One `.md` per command with **YAML frontmatter** (`description`, `allowed-tools`, `argument-hint`); slash name = filename stem. Plugin namespaces: `/praxion:co`. | **Plain Markdown only** — no frontmatter. Discovers from `.cursor/commands/` or `~/.cursor/commands/`. So repo `commands/*.md` must be **exported** (frontmatter stripped) for Cursor. |
