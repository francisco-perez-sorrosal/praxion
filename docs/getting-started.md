...
- `.ai-state/idea_ledgers/IDEA_LEDGER_*.md` — permanent ideation history
...
| **Architect-facing architecture** | `.ai-state/DESIGN.md` | systems-architect; updated by implementation-planner + implementer | Living document |
| **Developer-facing architecture** | `docs/architecture.md` | systems-architect (derived from `.ai-state/DESIGN.md`, Built components only) | Living document |
| **Sentinel audit reports** | `.ai-state/sentinel_reports/SENTINEL_REPORT_*.md` + `.ai-state/sentinel_reports/SENTINEL_LOG.md` | sentinel (independent, on demand) | Each run timestamped; accumulates |
...
3. Rewrites every `dec-draft-<hash>` reference to `dec-NNN` across the bounded walk scope: sibling ADRs, `.ai-work/*/LEARNINGS.md`, `.ai-work/*/SYSTEMS_PLAN.md`, `.ai-work/*/IMPLEMENTATION_PLAN.md`, `.ai-state/DESIGN.md`, `docs/architecture.md`, `scripts/*.py`, `scripts/*.sh`, and matching `.ai-state/specs/SPEC_*.md`
...
|---|-----------|---------|----------|
| 1 | **Working PoC + architecture baseline + first ADR** | The pipeline (Steps 1–7 above) + `/co` to commit | Green tests, committed code on a feature branch, plus `.ai-state/DESIGN.md`, `docs/architecture.md`, and one ADR draft under `.ai-state/decisions/drafts/` — the seed's systems-architect runs its full delegation checklist so the user's first pipeline observation includes Praxion's complete architecture artifact set |
| 2 | **Health audit** | `sentinel` agent — e.g., "Run a sentinel audit" | `.ai-state/sentinel_reports/SENTINEL_REPORT_*.md` (timestamped) + append-only sibling `.ai-state/sentinel_reports/SENTINEL_LOG.md` — coherence, freshness, completeness scorecard |
| 3 | **Architecture evolves** | `systems-architect` on each feature — lands at seed, updates with every subsequent pipeline | `.ai-state/DESIGN.md` + `docs/architecture.md` get **updated**, not replaced, as new modules land or dependencies shift |
