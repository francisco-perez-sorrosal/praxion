...
| **5 — Capstone** | integration eval over the criteria→spec path | R10 | ✅ **Done** (2026-06-29) |
...
**Wave 4 — fully closed (2026-06-29).** Every item has landed; the last deferred one, the **R12b auto-regen hook**, is now done *properly* (✅ **closed 2026-06-29** — see the R12b closure below). Its original deferral blamed `generated_at` churn, but the real blocker — found on disk — was that the committed manifest indexed **volatile gitignored `.ai-work/`**; the fix separates that volatile dimension from the committed durable index. (Earlier Wave-4 closures: **EA-06 · R9-dash · D2** 2026-06-29; **R8 · R19b · EA-09 · EA-10** 2026-06-28; R19a Wave 1; **EA-11 reader** ✅ 2026-06-29.) **With R12b closed, the §6 remediation backlog (R1–R19, EA-01..12) is effectively complete.**

**Wave 5 (R10) — capstone criteria→spec chain eval, landed 2026-06-29** (branch `wave5-r10-criteria-spec-eval`; full agent pipeline researcher→architect→planner→test-engineer→verifier; ADR draft → dec-258):

- ✅ **R10 / G1 closed** — new `tests/test_criteria_spec_eval.py` drives the criteria→spec chain (`TASK_BRIEF → SYSTEMS_PLAN → traceability.yml → SPEC archival`) end-to-end through the **four real validators** (`check_p06_task_brief.run_p06`, `spec_drift.detect_drift`, an inline REQ-flow/matrix check, `check_spec_archival_gap.detect_gap`) over `tmp_path` mini-repos: one faithful-chain test (all four clean **and** the same REQ IDs asserted across all four artifacts) + four per-link **bite canaries** (each breaks exactly one link and asserts the matching validator bites). dec-252 liveness proven — faithful PASSES, broken BITES — confirmed by an independent orchestrator bite-proof (7/7, incl. a control case isolating link-4's load-bearing stale-date break). No mocks (coverage confirms the real validators are exercised); everything built in `tmp_path` (no committed `.ai-work/` fixture — the EA-06 trap avoided); id-citation-clean via the `f"REQ-{n:02d}"` builder idiom (committed `tests/*.py` IS scanned — only `tests/fixtures/` is exempt).
- ✅ **Form decision (ADR dec-258)** — a pytest integration test in the **always-green suite** (form a), not a `/eval-praxion` check (b) or an `eval/` replay-harness (c): a gate-liveness proof must run free/deterministic/auth-less on every run, and (b)/(c) are out-of-band by design (re-affirms dec-040). `category: architectural` with mandatory Disconfirmation + `dissent` (the eval proves mechanical link-up, not the semantic artifact-faithfulness that remains the eval framework's charter).
- Verifier verdict: **PASS WITH FINDINGS** — 12/12 AC; W-01 (an out-of-scope `SPECS_INDEX.md` index-staleness edit from a dev-time regen) reverted to stay surgical; AC-07 = this §0.1 row + the calibration row are the merge-stage bookkeeping. Full suite **216** green (`tests/`) + **1086** (`scripts/`). Two behavioral-contract catches worth recording: the architect's **Register-Objection** corrected a wrong id-citation rationale inherited from research, and the orchestrator closed a REQ-flow "assumed-not-proven" gap inline (the faithful test now asserts the criteria thread across all four artifacts, not just the SPEC). *SPEC archival of this feature was deliberately deferred — it would bundle the unrelated index-staleness refresh and is not in the stated acceptance.*

**EA-11 reader closure — landed 2026-06-29** (branch `ea11-cleanup-policy-reader`; full agent pipeline architect→planner→test-engineer→implementer→verifier with an intra-step light-review at the RISKY one-way-door seam; ADR draft → dec-259):

- ✅ **EA-11 fully closed** — `clean_work_safety.py` now READS each artifact's deletion-severity class from `artifact_registry.cleanup_policy` (a read-only `cleanup_policy_for()` projection + a `_POLICY_SEVERITY` map + `_severity_for()`), instead of hardcoded `block`/`warn` literals. The registry's `cleanup_policy` field — populated since dec-251, **read by no one** until now (dec-256's recorded dissent + reversal-trigger) — is finally consumed. This was a **behavior-preserving** read-rewire of **safety-critical one-way-door deletion logic** (it governs what `/clean-work` deletes), done **characterization-tests-first**: pins of the exact pre-refactor BLOCK/WARN/SAFE verdicts were GREEN on the unchanged code first and stayed green, and the old-vs-new **dry-run diff over 22 live `.ai-work/` dirs is EMPTY** (4 BLOCK / 11 WARN / 7 SAFE, byte-identical). The read is proven **load-bearing** (a genuine-consumption test: overriding a registry policy flips the verdict BLOCK→WARN — mutation-confirmed by the verifier) and **self-guarding** (a set-equality drift test + `FAKE_ARTIFACT.md` canary catches a future registry non-`delete` artifact lacking a `clean_work_safety` predicate). Deletion-monotonic floor: an unknown/`archive` policy resolves to WARN, never SAFE.
- ✅ **Granularity decision (ADR dec-259) — re-affirms dec-256, not supersedes.** Option (a): the registry supplies the file→severity *class*; `clean_work_safety` keeps the per-file content-predicates (WIP unchecked-step, VERIFICATION merge-marker, PRE_REFACTOR `[CONSUMED]`, the traceability-vs-spec `elif`) and aggregates to the per-dir verdict exactly as before. Building the reader **discharges dec-256's reversal-trigger** ("if the per-artifact `cleanup_policy` reader never materialises … collapse both back") — confirming the registry-as-readable-spine bet rather than reversing it. Per the Re-affirmation Protocol: `re_affirms: dec-256`; dec-256 stays `accepted` and gains `re_affirmed_by`.
- Verifier verdict: **PASS WITH FINDINGS** — 14/14 AC ("no path found by which the change could delete a file it should not"); the two WARNs are orchestrator close-out (this §0.1 row + the calibration row) and an ephemeral `traceability.yml` backfill (moot — no SPEC archived). Full suite **1100** (`scripts/`) + **216** (`tests/`) green; intra-step light-review PASS WITH NITS (a cosmetic double-`_severity_for`-call per site, consciously deferred — re-touching the verified file for nil behavioral benefit would invalidate the empty-dry-run-diff safety evidence). *SPEC archival deliberately deferred (as in Wave 5): a faithful regen would bundle the unrelated `SPECS_INDEX.md` staleness.*

**R12b closure — `doc_manifest` volatile/durable split + auto-regen, landed 2026-06-29** (branch `r12b-manifest-volatile-split`; full agent pipeline researcher→architect→planner→implementer(×2)→verifier with an intra-step light-review at the RISKY finalize-hook seam; ADR draft → dec-draft-97093ff8):

- ✅ **R12b done properly** — the committed `.ai-state/doc_manifest.yaml` no longer indexes volatile gitignored `.ai-work/`. The deferral's stated rationale (`generated_at` churn) **understated the real blocker**, found on disk: the committed manifest carried **61 `.ai-work/` entries** that change with every pipeline — a fresh regen in a different worktree yielded **2**, i.e. the same `main` commit produced different manifests (a category error in *what the artifact indexes*, not mere staleness). The fix separates the two dimensions: the committed manifest indexes only **durable** surfaces (`docs/` + `.ai-state/`); the dashboard's **live** workshops view already owns `.ai-work/` (reads it from disk, manifest-independent — so removing the manifest's entries dropped only a permanently-stale docs-nav group, no functional capability). `build_doc_manifest` drops the `.ai-work/` walk + its registry import; a **content-aware write** (reusing the `--check` compare via a `_strip_generated_at` helper) skips the write when nothing durable changed (a no-op regen is byte-identical — no `generated_at` churn); and an **existence-gated** `build_doc_manifest` invocation in `finalize_chain.sh`'s `_finalize_chain_run_on_main` (after `finalize_adrs` + the ledger finalize) keeps the committed manifest fresh as the `dec-NNN` renames the finalize hook itself makes land — a pre-commit gate runs too early to see them. The existence gate (`[ -f .../doc_manifest.yaml ]`) keeps it surgical: it only *refreshes* an existing manifest, never *creates* one in a project that didn't opt in (light-review-confirmed cross-project-safe + non-blocking). Ground-truth-verified: a fresh build (from a canonical checkout) has **0 `.ai-work/`** surfaces and **349** durable surfaces, no surface dropped vs the pre-change code; the no-op write is byte-stable. F11 stays at WARN as a `--no-verify`/hook-failure backstop (it already scoped to `docs/ .ai-state/`).
- ✅ **dec-256/R18 coherence (ADR dec-draft-97093ff8) — new decision, no supersession.** Removing the `.ai-work/` walk removes `build_doc_manifest`'s `dashboard_artifacts_ordered()` import — dec-256/R18's "first consumer that *reads* the spine." Framed honestly as a **coherence improvement**: that read existed only to walk `.ai-work/` for the committed manifest (the volatile dimension being removed); the registry's `dashboard` projection still governs the dashboard's *live* discovery (drift-checked via `files.ts`), and the "registry is genuinely read" property is preserved by **dec-259**'s `clean_work_safety` reader. The now-dead `dashboard_artifacts_ordered()` helper was removed (zero callers) and the documented consumer count corrected **4 → 3** across the registry docstring, `scripts/CLAUDE.md`, `DESIGN.md`, and `docs/architecture.md` (registry **data** untouched; the `dissent` records that the genuine-reader property now rests solely on dec-259's reader).
- Verifier verdict: **PASS WITH FINDINGS** (no FAIL; "no way found for the committed manifest to carry volatile state, and no durable surface is dropped"). Full suite **1103** (`scripts/`) + **216** (`tests/`) green. The standout catch was a **close-out-location hazard the orchestrator's own checks missed**: `build_doc_manifest`'s pre-existing `.claude` exclusion means a regen run *from this worktree* (which lives under `.claude/worktrees/`) silently drops every `docs/` surface — so the manifest refresh must run from the **canonical checkout**; the new finalize hook does exactly that at merge (dogfooded). Orchestrator coherence catch: the architect's plan kept `dashboard_artifacts_ordered()`, but it became dead code (its sole caller, the build_doc_manifest import, was being removed) — the orchestrator removed it + a second `scripts/CLAUDE.md` "four consumers" site the plan missed. Residuals (deferred, not regressions): the pre-existing `.claude`-exclusion in-worktree-regen bug (worth a follow-up tech-debt row), and a stale `files.ts:11` comment citing the removed `_AI_WORK_FILES` (left under the correct no-touch-dashboard decision).
