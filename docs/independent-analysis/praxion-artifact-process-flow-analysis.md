...
**R12b closure — `doc_manifest` volatile/durable split + auto-regen, landed 2026-06-29** (branch `r12b-manifest-volatile-split`; full agent pipeline researcher→architect→planner→implementer(×2)→verifier with an intra-step light-review at the RISKY finalize-hook seam; ADR draft → dec-260):
