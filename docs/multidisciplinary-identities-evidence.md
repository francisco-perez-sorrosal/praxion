...
| Identity | Gap | Status |
|---|---|---|
| **evidence-appraiser** | Source provenance, study-design-vs-claim-strength, external validity and transfer, citation drift, vendor-claim quarantine | **SHIPPED as of `v0.19.0`/`v0.20.0`**, discipline #2, bound to `evidence-appraisal`. History: this row originally read "SHIPPED" before any release carried it; corrected 2026-07-30 (Wave 2) to "MERGED … NOT YET RELEASED" once it was confirmed the `v0.18.0` tag tree carried `skills/applied-statistics` but not `skills/evidence-appraisal` (zero `evidence-appraiser` registry rows) — a managed project on `v0.18.0` resolved `Discipline: evidence-appraiser` to `[BLOCKED]`, fail-loud as designed but not the advertised behaviour. That "NOT YET RELEASED" reading itself went stale once `v0.19.0` shipped the five `skills/evidence-appraisal/` files and a complete registry row (re-verified at `v0.20.0`) — `scripts/check_release_staleness.py` detected the staleness correctly and is CI-wired, but the check is advisory and nothing was named to consume its output before this sentence was written, so the row drifted from the gate's own output for one release cycle (`td-073`, second half: closed by naming `commands/release.md` as the check's required reader). Errors are silent: a misread import propagates carrying the authority of a number. Charter is concrete — the first live consult declined to verify the coefficients its own analysis rested on, recording them as *relayed* |
