...
Every pipeline run operates within `.ai-work/<task-slug>/`. The slug is **kebab-case** (2–4 words, e.g. `auth-flow`), generated by the **main agent** at pipeline start from the task description, included in every subagent prompt (`Task slug: <slug>`), used for all `.ai-work/` reads and writes, and inherited unchanged downstream — isolating each pipeline's working state from concurrent runs. Generation guidelines and the full propagation contract: [`coordination-details.md` § Task Slug Propagation](../../skills/software-planning/references/coordination-details.md#task-slug-propagation).
