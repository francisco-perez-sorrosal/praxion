...
| Hyphen-less `AC7`, `DS2`, `REQ10` | same as the dashed forms | Always ephemeral | **Never** |
| `<PIPELINE_DOC>.md § <section>` (e.g., `SYSTEMS_PLAN.md § Interfaces`) | any `.ai-work/` document in `scripts/artifact_registry.py` | Deleted with `.ai-work/` | **Never** — state the constraint inline, or cite the finalized ADR that carries it |
...
**Declared limits** — shapes the gate deliberately does not match, so the verifier's convention phase and review still carry them: `AC10`+ and `EC07`-style ids (indistinguishable from sentinel check ids, which this rule allows); roadmap hypothesis ids such as `F12` (collide with sentinel `F07`); bare pipeline filenames and concrete `.ai-work/<slug>/` paths (code legitimately reads those files, and tests build them as fixtures); single-letter criterion ids such as `A10` or `S3` (ordinary words).
