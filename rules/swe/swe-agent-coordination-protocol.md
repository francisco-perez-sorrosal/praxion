...
- **Intake Clarity Gate** (all tiers above Direct) — before tier-commit and before spawning the first agent, disambiguate intent. Surface gap-filling assumptions **univocally** (so the user can halt) and proceed; raise a *blocking* clarifying question (≤3, via `AskUserQuestion`) **only when intent is ambiguous AND a wrong guess is hard to reverse** — never on ambiguity alone. Where success is non-obvious, capture `.ai-work/<task-slug>/TASK_BRIEF.md` (Intent / Key Signals / Health Guards / Uncertainty Flag), seeding every downstream stage and the verifier's rubric. Direct tier skips it. Full procedure + the 2×2 decision rule: the `goal-disambiguation` skill.
