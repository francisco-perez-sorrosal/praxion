...
def _render_next_action(verdicts: Sequence[dict[str, Any]]) -> str:
    ...
    pending = [v for v in verdicts if v.get("verdict") != VERIFIED_COMPLETE]
    if not pending:
        return (
    ...
    nxt = pending[0]
    files = nxt.get("resume_scope") or nxt.get("tier1", {}).get("files_unchanged", [])
    scope = ", ".join(f"`{path}`" for path in files) or "see the plan's `Files:` field"
    return (
        f"`{nxt.get('step', '?')}` — {nxt.get('verdict', '?')}. File scope: {scope}.\n"
        f"Evidence: {nxt.get('evidence', '')}"
    )
