...
Portable project guidance for {title}.
...
This section should not survive Codex template generation.
{extra}
"""
...
def test_install_codex_exports_canonical_paths_and_check_passes(tmp_path: Path):
    ...
    agents_tmpl = (project_dir / "AGENTS.md.tmpl").read_text(encoding="utf-8")
    ...
    assert (
        str((REPO_ROOT / "skills" / "software-planning" / "SKILL.md").resolve()).replace("\\", "/")
        in planning
    )
    ...
    assert agents_md.startswith("<!-- Generated by Praxion Codex install")
    assert "# Development Guidelines for Codex" in agents_md
    assert "## Principles" in agents_md
    assert "## Project Layering" in agents_md
    assert "# Agent Instructions for Demo Project" in agents_tmpl
    assert "# Agent Instructions for Demo Project" in agents_md
    assert "Claude-Code-specific machinery" not in agents_tmpl
    ...
    assert (project_dir / ".codex" / "hooks" / "praxion-decisions-session-start.py").exists()
    ...
    assert (project_dir / ".codex" / "hooks" / "praxion-observability-post-tool-use.py").exists()
    ...
    assert agents_md.index("## Praxion Adapter") < agents_md.index(
        "# Agent Instructions for Demo Project"
    )
    ...
    assert "praxion-decisions-session-start.py" in hooks_text
    ...
    assert "project_doc_fallback_filenames" not in project_config
    chronograph_config = project_config["mcp_servers"]["task-chronograph"]
    assert "memory" not in project_config["mcp_servers"]
    assert chronograph_config["args"][2] == str((REPO_ROOT / "task-chronograph-mcp").resolve())
...
def test_install_codex_dry_run_reports_pipeline_adapter(tmp_path: Path):
    ...
    assert "Would generate" in dry_run.stdout
...
def test_install_codex_compat_only_skips_native_codex_surfaces(tmp_path: Path):
    ...
    assert (project_dir / "AGENTS.md.tmpl").exists()
    assert not (project_dir / ".codex").exists()
    assert not (project_dir / ".agents").exists()
    assert not (home_dir / ".codex").exists()


def test_install_codex_uses_existing_project_agents_template_and_restores_backup(
    tmp_path: Path,
):
    project_dir = tmp_path / "project"
    project_dir.mkdir()
    ...
    write_claude_md(project_dir, title="Ignored Claude Title")
    (project_dir / "AGENTS.md").write_text(
        "# Project Instructions\n\nAlways include PROJECT_ONLY_RULE.\n",
        encoding="utf-8",
    )
    (project_dir / "AGENTS.md.tmpl").write_text(
        "# Agent Instructions for Template Project\n\nAlways include PROJECT_TEMPLATE_RULE.\n",
        encoding="utf-8",
    )
    ...
    assert "PROJECT_TEMPLATE_RULE" in agents_md
    assert "PROJECT_ONLY_RULE" not in agents_md
    assert "Ignored Claude Title" not in agents_md
    assert (project_dir / ".praxion-codex-agents.backup").read_text(
        encoding="utf-8"
    ) == "# Project Instructions\n\nAlways include PROJECT_ONLY_RULE.\n"
    ...
    assert not (project_dir / ".praxion-codex-agents.backup").exists()
...
def test_install_codex_uninstall_preserves_user_codex_config_and_hooks(tmp_path: Path):
    ...
    shared_config = tomllib.loads((shared_codex_dir / "config.toml").read_text(encoding="utf-8"))
    ...
    restored_shared_config = (shared_codex_dir / "config.toml").read_text(encoding="utf-8")
