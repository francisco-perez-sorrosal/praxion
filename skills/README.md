...
| **[typescript-development](typescript-development/)** | TypeScript language patterns, code quality toolchain (ESLint, Prettier, tsc), and framework contexts (React, Vue). | Writing TypeScript code, implementing tests, configuring type checking, setting up linting and formatting, framework-specific development. |
...
Nine skills now include TypeScript language support via `contexts/` and `references/` alongside Python: `agent-evals`, `architectural-fitness-functions`, `deployment`, `llm-prompt-engineering`, `mcp-crafting`, `node-prj-mgmt`, `software-planning`, `test-coverage`, and `typescript-development`. TypeScript-specific content is progressively disclosed — load the skill and follow the language-specific reference for framework and toolchain details.
