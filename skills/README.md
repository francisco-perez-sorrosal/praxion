...
| **[test-coverage](test-coverage/)** | Language-agnostic dispatcher for locating, invoking, and rendering a project's test coverage consistently across terminals, Markdown reports, and verifier fragments. Dispatches to the project's own tooling (pytest-cov, etc.) via a convention-based probe order; never installs tooling. Python reference ships in v1. | Reporting coverage percentages, running a project's canonical coverage target, rendering coverage tables, comparing coverage against a prior run, wiring coverage into commands, agents, or metrics pipelines. |
