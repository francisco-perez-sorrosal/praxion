...
- **Prompt caching silently ignores blocks below threshold.** The minimum is per-model (512-4,096 tokens as of 2026-07-29, and shifts every release) — under-threshold `cache_control` markers produce no cache hit and no error, just an unexplained cost. Current table: `claude-ecosystem`'s [platform-services.md § Minimum Block Sizes](../claude-ecosystem/references/platform-services.md#minimum-block-sizes).
- **A cache-read breakpoint only looks back 20 blocks.** Placing it on a block that changes every request (a timestamp, a per-turn user message) never hits — silently, no error. Place the breakpoint on the last block that stays identical across the calls sharing the cache. See `claude-ecosystem`'s [platform-services.md § 20-Block Lookback Window](../claude-ecosystem/references/platform-services.md#the-20-block-lookback-window-highest-value-gotcha).
