...
4. **Cross-reference rewrite.** Rewrite every `dec-draft-<hash>` occurrence (for each promoted draft) to its newly assigned `dec-NNN` across one bounded **citation net** (the walk is bounded by design — finalize does not sweep the full repo, and never touches code). A citation that dangles because it sits *outside* the net is fixed by extending the net, not by forbidding the citation. The rewriter and the post-condition detector that runs after it share this single definition, so a citation the detector can see is one the rewriter already visited; a survivor is a failed rewrite of that file, never a missing location:
