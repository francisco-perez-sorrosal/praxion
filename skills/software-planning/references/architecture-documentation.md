...
- Section 1 (Overview): quick-facts table with Source stage, summary, and the `Current as of dec-NNN` checkpoint row (see **Checkpoint** below)
...
**Checkpoint:** Section 1's `Current as of` row is an authored high-water mark, not a computation — the systems-architect asserts *"I have considered every architecture-bearing decision up to `dec-NNN`"* when it advances. Considering a decision does not mean quoting it: advancing the checkpoint is the sanctioned disposition whether a decision changed the document ("folded in") or was judged not to require a change ("not applicable") — both outcomes advance the mark identically, and the mark never decreases. A derived, read-only validator (`scripts/check_design_checkpoint.py`) computes the un-folded suffix — architecture-bearing ADRs whose id sorts after the checkpoint — and reports it as an advisory count, never a gate. **The predicate that defines "architecture-bearing" lives in the script and only there**; this document never restates it in prose, because a predicate stated in two places is a predicate that will diverge. Three consumers surface the validator's output: the systems-architect's Phase 5 (which advances the mark after considering the un-folded suffix), `architect-validator` (the ADR leg of the code↔DSL↔ADR triangle), and the sentinel (an advisory finding, self-skipping when either `.ai-state/DESIGN.md` or `.ai-state/decisions/` is absent). No file is generated by any of this — the validator's only output is stdout. This is a deliberate departure from the regenerability contract elsewhere in these two documents: DESIGN.md's sections are hand-authored prose, not derived from the decision log, so "delete and rebuild from `.ai-state/decisions/`" does not apply to it the way regenerating `docs/architecture.md`'s diagrams from their LikeC4 source does — only the checkpoint's *validity* is mechanically re-derivable (by re-running the validator), never the document's content. Say this plainly rather than let a reader infer DESIGN.md is regenerable because a validator exists for it.
...
| ADR created | None for Section 8 — it is a stable pointer to `DECISIONS_INDEX.md` (auto-regenerated). When a new ADR materially shapes a component/interface/constraint, cite `dec-NNN` inline in the affected row instead. |
| Architecture-bearing ADR authored | 1 (Overview) — advance the `Current as of` checkpoint once the decision has been considered (folded in or judged not applicable). The predicate that identifies "architecture-bearing" is evaluated by `scripts/check_design_checkpoint.py`, never restated here — see **Checkpoint** above. |
...
- Section 8 contains a pointer to `DECISIONS_INDEX.md` — does not embed an ADR summary table. ADR currency (has the architect caught up with the log?) is a separate check against Section 1's `Current as of` checkpoint, not against Section 8
...
- **Section 1's `Current as of` checkpoint** is the document's ADR-currency signal — a high-water-mark `dec-NNN` naming the last decision the architect had considered when the document was last updated. It is orthogonal to Section 8's pointer: Section 8 says "here is where ADRs live"; the checkpoint says "here is how far this document has caught up." See **Checkpoint** under Document Lifecycle.
...
4. Section 8 needs no per-bootstrap population — it is a stable pointer to `DECISIONS_INDEX.md`. Inline `dec-NNN` references should be added in Sections 3/4/7 only when an existing ADR materially shaped a row. Section 1's checkpoint is a separate bootstrap step: set `Current as of` to the corpus tip (the newest ADR id at bootstrap time) — do not attempt to fold the historical backlog first; see **Checkpoint** under Document Lifecycle
