...
| Permanent | `.ai-state/` | `idea_ledgers/IDEA_LEDGER_*.md`, `sentinel_reports/{SENTINEL_REPORT_*.md, SENTINEL_LOG.md}`, `skill_genesis_reports/{SKILL_GENESIS_REPORT_*.md, SKILL_GENESIS_LOG.md}`, `metrics_reports/{METRICS_REPORT_*.{md,json}, METRICS_LOG.md}`, `specs/SPEC_*.md`, `calibration_log.md`, `decisions/<NNN>-<slug>.md`, `SYSTEM_DEPLOYMENT.md`, `DESIGN.md`, `TEST_TOPOLOGY.md`, `TECH_DEBT_LEDGER.md` (active) + `TECH_DEBT_RESOLVED.md` (terminal), `eval_ledger/EVAL_LOG.md`, `observations_summary.jsonl` (one committed rollup row per session, written atomically at Stop; the raw `observations.jsonl` WAL is local-only and gitignored), `token_budget_baseline.json` (dated always-loaded measurements + the frozen listing ceiling read by `measure_token_budget.py --ratchet`), `praxion_eval_reports/judge_cache.jsonl` (content-addressed judge verdicts, created lazily on the first judged run), `project_profile.yaml` | Project lifetime — committed to git, timestamped per run or living document |
