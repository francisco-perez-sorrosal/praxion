...
**Native isolation (harness-enforced).** The harness itself refuses `Write`/`Edit`/`NotebookEdit` — including through a symlink whose realpath leaves the worktree — that targets outside the session worktree; this needs no hook and cannot be turned off. The one gap it leaves: a Bash-path write (heredoc, script, `sh -c`) into the main checkout from a worktree session is not blocked mechanically. The compensating control is procedural, not a hook — pathspec-scoped commits, merge gates verified from the main checkout, and `git status` re-derivation before any resume, the same discipline this protocol already requires elsewhere.
...
7. **Durable disposition record.** Because `INTERFACE_DESIGN.md` / `TRANSACTIONS_DESIGN.md` are ephemeral (deleted with `.ai-work/<task-slug>/` at cleanup), the disposition of every non-empty `## Architecture Challenges` entry — `adopted` / `adopted-with-modification` / `rejected (reason)` / `escalated-to-user (outcome)` — is recorded in a **durable** artifact so the rationale survives cleanup: an adopted challenge by the architect folding it into `SYSTEMS_PLAN.md`; a rejected or escalated one by a one-line note in `SYSTEMS_PLAN.md § Architecture` (or `LEARNINGS.md` when it surfaced a reusable pattern). A populated `## Architecture Challenges` with no recorded disposition is a behavioral-contract violation, the same class as a silent accept/reject. The same record requirement applies to the agentic-transactions-architect's `TRANSACTIONS_DESIGN.md` challenge channel.
...
The convener — never the consultant — writes this, **after deciding to convene and before spawning**. That order is load-bearing: a convener that ran the lens pass first and only then decided whether to convene would systematically skip the consults where the lens had already found everything, which are precisely the observations the series most needs.

Having decided, the convener loads the matched registry row's `binds-to` skill(s) through the `Skill` tool and works that row's `challenge-obligations` against the draft — the consultant's own Round-1 checklist, run by one party instead of two. Whatever the pass surfaces is appended to `.ai-state/CONSULT_PRIORS.md § Sealed Priors`, one row per concern, on the same `(task-slug, discipline, stage)` triple the ledger and the cost series use, and the file is committed before the spawn. **An empty result is expressible and informative**: a single row whose `prior-id` is `NONE` and whose concern cell states what was run and what it found is a valid seal. An omitted seal is not.

The comparison group this establishes is deliberate. It is *not* what the convener happened to believe — it is what the convener produces **having read the discipline's own skill**, which is the counterfactual the lens-versus-consultant criterion is about. A list of un-primed intuitions understates the lens arm and inflates the consultant's apparent novelty, which is the direction that flatters the mechanism.

The convener may act on what the pass surfaces — fixing the draft before spawning is the point of running a lens at all, and a fixed defect can no longer be raised, so it correctly counts for neither party. What the convener may **not** do is edit sealed rows afterwards: the file is append-only, and a revision is a visible edit in git history.
