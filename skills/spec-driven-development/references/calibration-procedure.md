...
- **Retrospective**: filled post-pipeline — an enum value (`correct` tier was appropriate, `under-calibrated` should have been higher, `over-calibrated` should have been lower, `[pending]` not yet assessed), optionally followed by an em-dash and a one-sentence micro-capture note. `check_calibration_coverage.py` validates the enum prefix on rows dated on/after its cutover date; rows predating the cutover are exempt. This cell doubles as the Direct-tier sanctioned micro-journal: a learning, gotcha, debt item, or decision captured in one sentence at commit time — a sentence, not a section. Substantial items promote to an ADR or a `td-NNN` tech-debt-ledger row.
  - **At Standard/Full**, the verifier writes this enum plus its evidence clause into `VERIFICATION_REPORT.md`'s `## Calibration Verdict` section (an agent that read the finished plan and diff, not the agent that chose the tier); the orchestrator copies the verdict into the row unchanged.
  - **At Direct/Lightweight** (no verifier runs), the orchestrator writes the enum directly into the row, but must cite one concrete measurable rather than asserting `correct` bare — one of: a spawn count, a token figure (e.g. from a Step-10-style telemetry capture), or a mid-task re-tier event.
