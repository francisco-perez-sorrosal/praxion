...
- **Testing implementation instead of behavior.** Tests that assert on internal method calls, private state, or exact SQL queries break on every refactor without catching real bugs. Test through public interfaces and assert on observable outcomes. The design-time self-test: for every test, ask *"what incorrect behavior would still make this test pass?"* -- if the answer is "almost anything," the test pins structure, not behavior. A good test fails when the code does the wrong thing. (Mutation testing, under Coverage Philosophy below, is the automated form of this same question.)
