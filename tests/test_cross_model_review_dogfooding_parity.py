"""Dogfooding-parity test: Praxion's own cross-model-review caller stays a
faithful instance of the shipped `cross-model-review.yml.tmpl` caller
template.

`.github/workflows/cross-model-review.yml` (Praxion caller #1, dogfooding the
P2 gate on Praxion's own `ci-autofix/*` PRs) does not exist yet — only the hub
(`reusable-cross-model-review.yml`) and the still-commented-out template stub
exist at the time this module is written. This suite renders
`cross-model-review.yml.tmpl` with Praxion's own install-time values (the hub
owner/repo Praxion itself would receive from `/onboard-project`) and asserts
the installed caller is equivalent to the rendered result on every dimension
**except** the `uses:` ref line — the one documented, intentional divergence
(Praxion's caller tracks HEAD via a local `./...` ref; every rendered template
pins a cross-repo hub by SHA). Mirrors `test_ci_autofix_dogfooding_parity.py`.

Unlike the ci-autofix caller (whose `on.workflow_run.workflows` list is a
per-repo tunable), the cross-model-review caller's `on.pull_request.types`
list is a fixed literal with no per-repo divergence — so the `on:` trigger
block is asserted for exact equality between the installed caller and the
rendered template, with no substitution needed on that block.

Running this module now is expected to fail (RED): the installed caller file
does not exist, and the template it would be rendered against is still the
commented-out P2 placeholder stub — both the realized template and the
installed caller are pending implementation.
"""
...
CALLER_TEMPLATE_FILE = (
    PROJECT_ROOT / "claude" / "project-baseline" / "ci-autofix" / "cross-model-review.yml.tmpl"
)
INSTALLED_CALLER_FILE = PROJECT_ROOT / ".github" / "workflows" / "cross-model-review.yml"
...
_RENDER_SUBSTITUTIONS = {
    ...
    # line is the one documented, tolerated divergence. Any well-formed-
    # looking 40-hex value keeps the rendered template parseable.
    "{{HUB_SHA}}": "a" * 40,
}

_HUB_WORKFLOW_PATH_PATTERN = re.compile(r"(\.github/workflows/reusable-cross-model-review\.yml)")
...
def _installed_caller_parsed() -> dict:
    """Parse Praxion's installed caller (`.github/workflows/cross-model-review.yml`) as YAML."""
...
def _job_sans_uses(job: dict) -> dict:
    ...
    template (a cross-repo SHA-pinned ref). Every other job-level key must
    match exactly for the two to be considered a faithful instance of one
    another.
    """
...
def test_installed_caller_declares_only_the_same_top_level_keys_as_the_rendered_template() -> None:
    ...
    assert set(actual.keys()) == set(rendered.keys()), (
        ...
        "signals logic that should live in the hub, not the thin caller."
    )


def test_installed_callers_pull_request_trigger_matches_what_the_template_renders() -> None:
    rendered_on = _on_block(_rendered_template_parsed())
    actual_on = _on_block(_installed_caller_parsed())
    assert actual_on == rendered_on, (
        "The installed caller's `on.pull_request` trigger must equal the "
        f"template's exactly — got installed={actual_on!r}, rendered={rendered_on!r}"
    )
...
def test_installed_caller_job_matches_the_rendered_template_job_on_every_key_except_the_uses_ref() -> (
    ...
    assert _job_sans_uses(actual_job) == _job_sans_uses(rendered_job), (
        ...
        "differently-valued key (e.g. a widened permissions grant or a "
        "missing `with:`/`secrets:` mapping) signals the caller has drifted "
        f"from the shipped template's security contract. "
        f"installed(sans uses)={_job_sans_uses(actual_job)!r}, "
...
def test_installed_caller_and_template_invoke_the_same_reusable_workflow_file() -> None:
    ...
    assert actual_uses is not None, (
        ...
        "invoking the cross-model-review hub reusable workflow"
    )
    rendered_path = _reusable_workflow_relative_path(rendered_uses)
    actual_path = _reusable_workflow_relative_path(actual_uses)
    assert actual_path == rendered_path == ".github/workflows/reusable-cross-model-review.yml", (
...
def test_installed_caller_pins_the_hub_via_a_local_ref_while_the_template_uses_a_cross_repo_pin() -> (
    ...
    """Documents the one intentional divergence: Praxion's caller #1 tracks
    HEAD via a same-repo local ref; the shipped template ships the cross-repo
    SHA-pinned form every managed caller receives.
    """
    rendered_uses = _job(_rendered_template_parsed()).get("uses", "")
    actual_uses = _job(_installed_caller_parsed()).get("uses")
    assert (
...
def test_installed_callers_job_permissions_ceiling_matches_the_templates_inverted_privilege_block() -> (
    ...
    assert actual_permissions == _EXPECTED_JOB_PERMISSIONS == rendered_permissions, (
        ...
        f"inverted-privilege ceiling exactly — expected {_EXPECTED_JOB_PERMISSIONS}, "
        f"got installed={actual_permissions!r}, rendered={rendered_permissions!r}"
    )


def test_installed_caller_passes_the_cursor_api_key_by_explicit_mapping_never_inherit() -> None:
    rendered_secrets = _job(_rendered_template_parsed()).get("secrets")
    actual_secrets = _job(_installed_caller_parsed()).get("secrets")
    expected = {"CURSOR_API_KEY": "${{ secrets.CURSOR_API_KEY }}"}
    assert actual_secrets == expected == rendered_secrets, (
        "The installed caller's job must map CURSOR_API_KEY explicitly, "
        f"identically to the template — expected {expected}, got "
        f"installed={actual_secrets!r}, rendered={rendered_secrets!r}"
    )
    assert "secrets: inherit" not in _raw_text(INSTALLED_CALLER_FILE), (
        ...
        "exercises an explicit mapping even though it happens to be same-repo"
    )
